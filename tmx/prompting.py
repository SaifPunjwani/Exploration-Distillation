...
def build_generation_messages(prompt: str, cfg, phase: str = "eval") -> list[dict[str, str]]:
    ...
    if few_shot is None:
        import os
        few_shot = int(os.environ.get("TMX_FEW_SHOT_EXAMPLES", "0") or "0")
...
def get_answer_token_constraints(tokenizer, cfg, phase: str = "eval"):
    ...
    cached = getattr(tokenizer, "_tmx_answer_constraints", {})
    ...
    tokenizer._tmx_answer_constraints = cached
    return constraints
...
def _chat_template_kwargs(cfg=None, phase: str = "eval") -> dict:
    ...
    if "enable_thinking" not in kwargs:
        thinking_env = os.environ.get("TMX_ENABLE_THINKING", "").strip().lower()
