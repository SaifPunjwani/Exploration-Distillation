...
def _xla_metric_cpu_max_numel() -> int:
    raw = (os.environ.get("TMX_XLA_METRIC_CPU_MAX_NUMEL") or "").strip()
    if not raw:
        return 65536
    try:
...
def _xla_cpu_input_handoff_enabled() -> bool:
    return (os.environ.get("TMX_XLA_CPU_INPUT_HANDOFF", "1") or "").strip().lower() in (
        "1",
        "true",
        "yes",
    )
...
def hf_from_pretrained_kwargs(repo_id: str, repo_type: str = "model"):
    if (os.environ.get("TMX_HF_LOCAL_ONLY") or "").strip().lower() in ("1", "true", "yes"):
        return {"local_files_only": True}
    if (os.environ.get("TMX_HF_LOCAL_IF_CACHED", "1") or "").strip().lower() in ("0", "false", "no"):
        return {}
...
def patch_torch_autocast_enabled_signature():
    ...
    if fn is None:
        return
    if getattr(fn, "_tmx_accepts_device_type", False):
        return
    ...
    _wrapped_is_autocast_enabled._tmx_accepts_device_type = True  # type: ignore[attr-defined]
...
def causal_lm_from_pretrained(model_name: str, *, is_xla: bool = False, **kwargs):
    ...
    use_flash = is_xla and os.environ.get("TMX_XLA_FLASH_ATTENTION", "0") not in ("0", "false", "no", "")
    # FSDP + bf16 incompatibility: torch_xla FSDP requires fp32 params at wrap time.
    # Load fp32 when FSDP is active; FSDP's compute_dtype=bf16 handles bf16 computation.
    use_fsdp = os.environ.get("TMX_FSDP", "0") not in ("0", "false", "no", "")
    flash_load_dtype = torch.float32 if use_fsdp else torch.bfloat16
    if use_flash and _register_tpu_flash_attention():
        ...
        load_kwargs.setdefault("torch_dtype", flash_load_dtype)
        try:
            model = AutoModelForCausalLM.from_pretrained(model_name, **load_kwargs)
            model.config._attn_implementation = "tpu_flash"
            model.config._attn_implementation_internal = "tpu_flash"
            # Apply bf16 patches regardless of FSDP: the patch replaces HF RMSNorm/RotaryEmbedding
            # forward methods to skip their hardcoded fp32 upcast. Flash attention's Pallas kernel
            # requires bf16 throughout. With FSDP compute_dtype=bf16, params are cast to bf16 at
            # forward time; the patch ensures intermediate ops stay bf16 too.
            _patch_model_bf16_for_flash_attention(model)
            print(f"[patch] Loading causal LM with attn_implementation=tpu_flash, dtype={flash_load_dtype} for XLA (use_fsdp={use_fsdp}).")
            return model
    ...
    force_eager = is_xla and os.environ.get("TMX_XLA_FORCE_EAGER_ATTN", "1") != "0"
    if force_eager:
        load_kwargs.setdefault("attn_implementation", "eager")
        try:
            model = AutoModelForCausalLM.from_pretrained(model_name, **load_kwargs)
...
def patch_torch_checkpoint_autocast_for_xla():
    ...
    if getattr(checkpoint_mod, "_tmx_xla_autocast_patched", False):
        return
    ...
    checkpoint_mod._tmx_xla_autocast_patched = True
...
def patch_torch_xla_device_module_for_rng():
    ...
    """
    if getattr(getattr(torch, "xla", None), "_tmx_rng_registered", False):
        return
...
def patch_accelerate_gather_object_for_xla():
    ...
    if current is None:
        return
    if getattr(current, "_tmx_xla_safe", False):
        return
    ...
    gather_object_xla_safe._tmx_xla_safe = True  # type: ignore[attr-defined]
    accel_ops.gather_object = gather_object_xla_safe

    current_gather = getattr(Accelerator, "gather", None)
    if current_gather is not None and not getattr(current_gather, "_tmx_xla_single_worker_safe", False):
        ...
        gather_xla_safe._tmx_xla_single_worker_safe = True  # type: ignore[attr-defined]
    ...
    if current_gather_for_metrics is not None and not getattr(current_gather_for_metrics, "_tmx_xla_single_worker_safe", False):
        ...
        def gather_for_metrics_xla_safe(self, input_data, use_gather_object=False):
            ...
            local_metric_handoff = (
                ...
                and os.environ.get("TMX_XLA_LOCAL_METRIC_HANDOFF", "1") in ("1", "true", "True", "yes", "YES")
            ...
            if _single_worker_xla_collective_passthrough() and not use_gather_object:
                ...
                if os.environ.get("TMX_XLA_PLACEHOLDER_METRICS", "0") in ("1", "true", "True", "yes", "YES"):
        ...
        gather_for_metrics_xla_safe._tmx_xla_single_worker_safe = True  # type: ignore[attr-defined]
...
def patch_transformers_nested_xla_mesh_reduce_for_single_worker():
    ...
    if original is None:
        return
    if getattr(original, "_tmx_xla_single_worker_safe", False):
        return
    ...
    nested_xla_mesh_reduce_single_worker._tmx_xla_single_worker_safe = True  # type: ignore[attr-defined]
...
def patch_accelerate_gradient_state_for_xla():
    ...
    if original is None or getattr(original, "_tmx_patched", False):
        pass
    else:
        def training_step_with_mark_step(self, model, inputs, num_items_in_batch=None):
            ...
            if (
                ...
                and os.environ.get("TMX_XLA_PLACEHOLDER_STEP_LOSS", "0") in ("1", "true", "True", "yes", "YES")
            ):
        ...
        training_step_with_mark_step._tmx_patched = True  # type: ignore[attr-defined]
    ...
    if accelerate_state is not None:
        ...
        if original_set_sync_gradients is not None and not getattr(original_set_sync_gradients, "_tmx_patched", False):
            def _set_sync_gradients_xla_safe(self, sync_gradients):
                skip_xla_mark_step = (
                    ...
                    and os.environ.get("TMX_XLA_SKIP_ACCELERATE_SYNC_MARK_STEP", "0")
            ...
            _set_sync_gradients_xla_safe._tmx_patched = True  # type: ignore[attr-defined]
...
def patch_trl_grpo_compute_loss_for_xla():
    ...
    if original is None or getattr(original, "_tmx_xla_cpu_handoff", False):
        return
    ...
    _compute_loss_xla_safe._tmx_xla_cpu_handoff = True  # type: ignore[attr-defined]
...
def patch_transformers_accelerator_num_processes_for_xla_spmd():
    ...
    if original is None or getattr(original, "_tmx_xla_spmd_num_processes", False):
        return

    def create_accelerator_and_postprocess_xla_spmd(self, *args, **kwargs):
        result = original(self, *args, **kwargs)
        if _single_worker_xla_collective_passthrough() and (os.environ.get("TMX_FSDP") or "").strip().lower() in ("1", "true", "full_shard"):
    ...
    create_accelerator_and_postprocess_xla_spmd._tmx_xla_spmd_num_processes = True  # type: ignore[attr-defined]
...
def patch_trl_grpo_sampler_for_xla_spmd():
    ...
    if original is None or getattr(original, "_tmx_xla_spmd_sampler", False):
        return
    ...
    def _spmd_effective_num_processes(trainer) -> int:
        ...
        if (
            _single_worker_xla_collective_passthrough()
            and (os.environ.get("TMX_FSDP") or "").strip().lower() in ("1", "true", "full_shard")
        ):
            runtime_count = _xla_runtime_device_count()
    ...
    _get_train_sampler_xla_spmd._tmx_xla_spmd_sampler = True  # type: ignore[attr-defined]
...
def patch_transformers_trainer_logging_for_single_worker():
    ...
    _maybe_log_save_evaluate_xla_safe._tmx_xla_single_worker_safe = True  # type: ignore[attr-defined]
...
def patch_torch_xla_parallel_loader_for_single_worker():
    ...
    def mp_device_loader_init_xla_safe(self, loader, device, **kwargs):
        if _single_worker_xla_collective_passthrough():
            kwargs.setdefault(
                ...
                _env_int("TMX_XLA_LOADER_PREFETCH_SIZE", 1),
            )
            kwargs.setdefault(
                "device_prefetch_size",
                _env_int("TMX_XLA_DEVICE_PREFETCH_SIZE", 1),
            )
            kwargs.setdefault(
                "host_to_device_transfer_threads",
                _env_int("TMX_XLA_HOST_TO_DEVICE_TRANSFER_THREADS", 1),
            )
    ...
    def per_device_loader_next_xla_safe(self):
        ...
        skip_loader_mark_step = os.environ.get("TMX_XLA_SKIP_LOADER_MARK_STEP", "0") not in (
            "",
            "0",
            "false",
            "False",
    ...
    mp_device_loader_init_xla_safe._tmx_xla_single_worker_safe = True  # type: ignore[attr-defined]
    per_device_loader_next_xla_safe._tmx_xla_single_worker_safe = True  # type: ignore[attr-defined]
...
def xla_safe_generate(model, *args, **kwargs):
    ...
    debug_generate = os.environ.get("TMX_DEBUG_GENERATE", "0") == "1"
    try:
        progress_every = int(os.environ.get("TMX_XLA_GENERATE_PROGRESS_EVERY", "0") or "0")
    except Exception:
        progress_every = 0
    if progress_every < 0:
        progress_every = 0
    ...
    if _is_xla_device(device) and not use_incremental_xla:
        ...
        with torch.no_grad():
            ...
            mark_step_every = int(os.environ.get("TMX_XLA_MARK_STEP_EVERY", "1"))
    ...
    if _is_xla_device(device):
        try:
            ...
            xm = None
        xla_mark_step_every = int(os.environ.get("TMX_XLA_MARK_STEP_EVERY", "1"))
...
def patch_model_generate_for_xla(model):
    ...
    def _generate(self, *args, **kwargs):
        constraints = getattr(self, "_tmx_answer_constraints", None)
        if constraints:
...
def patch_tokenizer_for_xla_fixed_padding(tokenizer, max_length: int, warning_max_length: Optional[int] = None):
    """
    ...
    tokenizer._tmx_xla_fixed_padding_max_length = int(max_length)
    tokenizer.model_max_length = int(warning_max_length or max_length)

    tokenizer_cls = tokenizer.__class__
    if not getattr(tokenizer_cls, "_tmx_xla_fixed_padding_patched", False):
        original_call = tokenizer_cls.__call__

        def _call(self, *args, **kwargs):
            fixed_max_length = getattr(self, "_tmx_xla_fixed_padding_max_length", None)
        ...
        tokenizer_cls._tmx_xla_fixed_padding_patched = True

    if hasattr(tokenizer_cls, "apply_chat_template") and not getattr(tokenizer_cls, "_tmx_xla_chat_template_patched", False):
        ...
        tokenizer_cls._tmx_xla_chat_template_patched = True
