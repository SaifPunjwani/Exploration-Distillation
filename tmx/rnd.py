...
def _skip_train_on_xla() -> bool:
    # Safety flag: when set, RND runs forward-only on XLA (no backward,
    # no optimizer step). Avoids hangs from xm.optimizer_step(barrier=True)
    # if XLA lazy-tensor queue gets stuck. Novelty signal is still produced
    # from the fixed random target.
    v = os.environ.get("TMX_NOVELTY_SKIP_TRAIN_ON_XLA", "0").strip().lower()
...
class RNDModule(nn.Module):
    ...
    def compute_novelty(self,
        ...
        effective_train = train and not (self.is_xla and _skip_train_on_xla())
