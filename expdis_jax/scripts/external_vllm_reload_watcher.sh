...
RETRY_SECONDS="${EXPDIS_EXTERNAL_VLLM_RELOAD_RETRY_SECONDS:-30}"
MAX_ATTEMPTS="${EXPDIS_EXTERNAL_VLLM_RELOAD_MAX_ATTEMPTS:-0}"
SSH_CONNECT_TIMEOUT="${EXPDIS_EXTERNAL_RELOAD_SSH_CONNECT_TIMEOUT:-10}"
HEARTBEAT_EVERY_POLLS="${EXPDIS_EXTERNAL_RELOAD_HEARTBEAT_EVERY_POLLS:-60}"
...
wait_remote_server_health() {
  ...
  timeout_s="${EXPDIS_EXTERNAL_RELOAD_REMOTE_HEALTH_TIMEOUT:-300}"
  deadline=$(( $(date +%s) + timeout_s ))
  escaped_urls="$(printf '%q' "$server_urls")"
  while [ "$(date +%s)" -lt "$deadline" ]; do
    min_healthy="${EXPDIS_EXTERNAL_RELOAD_MIN_HEALTHY_URLS:-0}"
...
'"; then
    ...
    sleep "${EXPDIS_EXTERNAL_RELOAD_REMOTE_HEALTH_POLL_SECONDS:-5}"
  done
  return 1
}
...
wait_remote_server_completions() {
  ...
  timeout_s="${EXPDIS_EXTERNAL_RELOAD_REMOTE_COMPLETION_TIMEOUT:-600}"
  poll_s="${EXPDIS_EXTERNAL_RELOAD_REMOTE_COMPLETION_POLL_SECONDS:-10}"
  probe_timeout="${EXPDIS_EXTERNAL_RELOAD_COMPLETION_PROBE_TIMEOUT:-90}"
  deadline=$(( $(date +%s) + timeout_s ))
  escaped_urls="$(printf '%q' "$server_urls")"
  escaped_model="$(printf '%q' "$served_model_name")"
  while [ "$(date +%s)" -lt "$deadline" ]; do
    min_healthy="${EXPDIS_EXTERNAL_RELOAD_MIN_HEALTHY_URLS:-0}"
    if ssh "${SSH_OPTS[@]}" "$REMOTE_HOST" "SERVER_URLS=$escaped_urls SERVED_MODEL_NAME=$escaped_model PROBE_TIMEOUT=$probe_timeout MIN_HEALTHY_URLS=$min_healthy PROBE_CONCURRENCY=${EXPDIS_EXTERNAL_RELOAD_COMPLETION_PROBE_CONCURRENCY:-16} python3 -c '
...
while true; do
  ...
  while IFS= read -r request_path; do
    ...
    tmp_json="$(mktemp /tmp/expdis_reload_request_json.XXXXXX)"
    tmp_log="$(mktemp /tmp/expdis_reload_request_log.XXXXXX)"
    ...
    if [ -z "$tpu_project" ]; then
      ...
      tpu_project="${EXPDIS_TPU_PROJECT:-${CLOUDSDK_CORE_PROJECT:-}}"
    fi
    attempt="$(
      ssh "${SSH_OPTS[@]}" "$REMOTE_HOST" \
    ...
    if (
      ...
      LOCAL_MODEL_DIR_BASENAME="expdis_external_${stage}" \
      MAX_MODEL_LEN="${max_model_len:-10240}" \
      MAX_NUM_SEQS="${EXPDIS_TRAIN_RELOAD_VLLM_MAX_NUM_SEQS:-32}" \
      MAX_NUM_BATCHED_TOKENS="${EXPDIS_VLLM_RELOAD_MAX_BATCHED_TOKENS:-65536}" \
      EXPDIS_VLLM_MULTI_SERVER="${multi_server:-${EXPDIS_VLLM_MULTI_SERVER:-0}}" \
      CHIPS="${chips:-${EXPDIS_VLLM_MULTI_SERVER_CHIPS:-0 1 2 3}}" \
      PORT_BASE="${port_base:-${EXPDIS_VLLM_MULTI_SERVER_PORT_BASE:-8000}}" \
      VLLM_TPU_BUCKET_PADDING_GAP="${EXPDIS_VLLM_TPU_BUCKET_PADDING_GAP:-${VLLM_TPU_BUCKET_PADDING_GAP:-}}" \
      VLLM_TPU_MOST_MODEL_LEN="${EXPDIS_VLLM_TPU_MOST_MODEL_LEN:-${VLLM_TPU_MOST_MODEL_LEN:-}}" \
      VLLM_XLA_CACHE_PATH="${EXPDIS_VLLM_XLA_CACHE_PATH:-${VLLM_XLA_CACHE_PATH:-}}" \
      VLLM_V1_USE_PREFILL_DECODE_ATTENTION="${EXPDIS_VLLM_V1_USE_PREFILL_DECODE_ATTENTION:-${VLLM_V1_USE_PREFILL_DECODE_ATTENTION:-0}}" \
      VLLM_XLA_CACHE_TAG="${EXPDIS_VLLM_XLA_CACHE_TAG:-${VLLM_XLA_CACHE_TAG:-}}" \
      EXPDIS_VLLM_SKIP_FAILED_WORKERS="${EXPDIS_VLLM_SKIP_FAILED_WORKERS:-0}" \
      EXPDIS_VLLM_RELOAD_MIN_HEALTHY_WORKERS="${EXPDIS_VLLM_RELOAD_MIN_HEALTHY_WORKERS:-0}" \
      DISABLE_LOG_REQUESTS="${EXPDIS_VLLM_DISABLE_LOG_REQUESTS:-1}" \
      PARALLELISM="${EXPDIS_VLLM_RELOAD_PARALLELISM:-8}" \
      bash expdis_jax/scripts/reload_vllm_direct_slice.sh
    ) >"$tmp_log" 2>&1; then
      ...
      if [ "${EXPDIS_EXTERNAL_RELOAD_SKIP_COMPLETION_PROBE:-0}" = "1" ]; then
        printf '%s\n' "completion probe skipped by EXPDIS_EXTERNAL_RELOAD_SKIP_COMPLETION_PROBE=1" >>"$tmp_log"
