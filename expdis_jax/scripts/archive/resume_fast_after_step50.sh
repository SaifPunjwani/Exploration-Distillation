...
REMOTE_REPO="${REMOTE_REPO:?set REMOTE_REPO to the absolute repo path on the training VM}"
REMOTE_RUNS_ROOT="${REMOTE_RUNS_ROOT:-$REMOTE_REPO/runs}"
TPU_NAME="${TPU_NAME:?set TPU_NAME to the serving TPU VM name}"
ZONE="${ZONE:?set ZONE to the serving TPU zone}"
...
remote "bash -lc '
  ...
  pkill -9 -f \"[p]ython -m expdis_jax.pipeline\" 2>/dev/null || true
  sleep 5
  cd \"$REMOTE_REPO\"
  source ~/miniforge3/bin/activate expdis_jax
  export RUN_NAME=\"\$RUN_NAME\"
  export RUNS_ROOT=\"$REMOTE_RUNS_ROOT\"
  export INIT_CHECKPOINT=\"\$CKPT\"
  export EXPDIS_RESTORE_OPT_STATE=0
  export EXPDIS_SAVE_OPT_STATE=0
  ...
  export EXPDIS_WANDB_SINGLE_RUN=1
  export EXPDIS_WANDB_COMPACT=1
	  export EXPDIS_VLLM_RETURN_LOGPROBS=0
	  export EXPDIS_TRACK_EXTERNAL_OLD_GAP=0
	  export EXPDIS_TRAIN_VLLM_RELOAD_EVERY_STEPS=1
	  export EXPDIS_TRAIN_VLLM_RELOAD_ON_RESUME=1
	  export EXPDIS_TRAIN_VLLM_RELOAD_MODE=external
	  export EXPDIS_EXTERNAL_VLLM_RELOAD_WAIT=1
	  export EXPDIS_EXTERNAL_VLLM_RELOAD_REQUIRED=1
	  export EXPDIS_VLLM_RELOAD_SERVER_URLS=\"\$VLLM_URLS\"
	  export EXPDIS_VLLM_RELOAD_TPU_NAME=\"$TPU_NAME\"
	  export EXPDIS_VLLM_RELOAD_ZONE=\"$ZONE\"
	  export EXPDIS_VLLM_RELOAD_WORKERS=\"$WORKERS\"
  ...
  export EXPDIS_SKIP_FINAL_EVAL=\"\${PROOF_SKIP_FINAL_EVAL:-1}\"
  if [ -n \"\$WANDB_ID\" ]; then
    export EXPDIS_WANDB_RUN_ID=\"\$WANDB_ID\"
    export EXPDIS_WANDB_EXPLORER_RUN_ID=\"\$WANDB_ID\"
    export EXPDIS_WANDB_DISTILL_RUN_ID=\"\$WANDB_ID\"
    export EXPDIS_WANDB_ACTUAL_RUN_ID=\"\$WANDB_ID\"
    export EXPDIS_WANDB_ACTUAL_GRPO_RUN_ID=\"\$WANDB_ID\"
  fi
  tmux kill-session -t \"\${RUN_NAME}_resume_fast\" 2>/dev/null || true
  tmux new-session -d -s \"\${RUN_NAME}_resume_fast\" \"bash expdis_jax/scripts/launch_jax_pipeline.sh >> '\$ROOT'/launcher.log 2>&1\"
