...
PY
      grep -E \"actual_hf_final|EXPDIS_SKIP_FINAL_EVAL|pipeline/actual_hf_final_saved|Traceback|ERROR\" \"\$ROOT/launcher.log\" 2>/dev/null | tail -8
    '" 2>&1 || true
  )"
  log "$state"
...
remote "bash -lc '
  ...
  export EXPDIS_SKIP_FINAL_EVAL=\"\${LAMBDA05_SKIP_FINAL_EVAL:-1}\"
  ...
  tmux new-session -d -s \"\$RUN_NAME\" \"bash expdis_jax/scripts/launch_jax_pipeline.sh > '\$ROOT'/launcher.log 2>&1\"
