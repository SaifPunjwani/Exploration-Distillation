...
def main() -> None:
    ...
    parser.add_argument("--project", default=os.environ.get("WANDB_PROJECT", "expdis"))
    ...
    run = wandb.init(
        ...
        id=run_id,
        resume="allow",
        config={"source": "expdis_jax/scripts/mirror_wandb_from_log.py", "log_path": str(log_path)},
    )
    try:
        ...
        pass
    explicit_step = (os.environ.get("EXPDIS_WANDB_MIRROR_EXPLICIT_STEP", "0") or "").lower() in {
        "1",
        "true",
        "yes",
        "on",
