...
#
# Usage:
#   ./expdis_jax/scripts/monitor_tpu_run.sh HOST RUN_NAME [INTERVAL_SECONDS]
#
...
proc="$(pgrep -af "python -m expdis_jax\.(pipeline|train).*$RUN" | head -1 || true)"
if [ -n "$proc" ]; then
...
if [ -n "$wandb_id" ]; then
  echo "wandb: https://wandb.ai/${WANDB_ENTITY:-<entity>}/${WANDB_PROJECT:-expdis}/runs/$wandb_id"
fi
