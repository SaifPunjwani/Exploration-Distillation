...
def _download(model_name: str, cache_dir: str | None = None, *, weights: bool = True) -> str:
    if os.path.isdir(model_name):
        return os.path.abspath(model_name)
    return snapshot_download(
        repo_id=model_name,
        allow_patterns=["*.json", "*.tokenizer*", "*.model", "*.jinja", "merges.txt", "vocab.txt"]
        + (["model*.safetensors"] if weights else []),
        cache_dir=cache_dir,
    )


def _load_shards(snapshot_dir: str) -> Dict[str, np.ndarray]:
    """Load the HF checkpoint, ignoring native-format duplicates and sidecars."""
    ...
    if os.path.isfile(index_path):
        ...
        raise FileNotFoundError(f"No HF model.safetensors or shard index in {snapshot_dir}")
    tensors: Dict[str, np.ndarray] = {}
    for fname in filenames:
        if os.path.basename(fname) != fname:
            raise ValueError(f"HF shard filename must be a basename: {fname!r}")
        with safe_open(os.path.join(snapshot_dir, fname), framework="np") as f:
            for key in f.keys():
                if weight_map is None or weight_map.get(key) == fname:
...
def hf_to_flax_params(
    ...
    *,
    ...
    """
    if load_hf_config(model_name).get("quantization_config"):
        raise ValueError("quantized checkpoints are unsupported; use the full-precision BF16 checkpoint")
    snap = _download(model_name)
    sd = _load_shards(snap)
    quant_keys = [k for k in sd if k.endswith(("weight_scale_inv", "weight_scale", "activation_scale"))]
    if quant_keys:
        raise ValueError(
            "quantized (fp8/int) checkpoints are not supported by this loader; dequantize to bf16 first. "
            f"Example keys: {quant_keys[:3]}"
        )
    ...
    elif not tie_word_embeddings:
        raise KeyError(f"untied model but {lm_head_key!r} is missing from the checkpoint")
...
def export_flax_params_to_hf_dir(
    ...
    *,
    ...
    snap = _download(model_name, cache_dir=cache_dir, weights=False)
