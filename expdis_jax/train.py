...
Design:
  ...
  * vLLM HTTP for generation. The serving copy is synchronized to the trainer
    before every rollout batch (``EXPDIS_TRAIN_VLLM_RELOAD_EVERY_STEPS=1``), so
    rollouts and their old log-probs come from the policy snapshot frozen for
    that batch. With one optimizer step per batch the snapshot's log-probs equal
    the current-policy log-probs at the start of the step, so they are evaluated
    inline (``old_lp = stop_gradient(new_lp)``; ``make_rescore_step`` rescores
    them once per batch when ``grpo_updates_per_rollout > 1``). The ratio starts
    at 1, so the asymmetric 0.2/0.28 clip normally does not bind. vLLM log-probs
    are never used.
  * Checkpoints via Orbax every ``save_every_steps`` (50) updates, traj JSONL
    persisted to `cfg.output_dir`. The checkpoint/export cadence is independent
    of the per-batch sampler synchronization.
"""
...
# Reuse compiled shapes across restarts, including direct module invocations.
# EXPDIS_DISABLE_COMPILE_CACHE=1 disables the persistent cache.
if os.environ.get("EXPDIS_DISABLE_COMPILE_CACHE", "0").lower() not in ("1", "true", "yes", "on"):
    _COMPILE_CACHE_DIR = os.environ.get(
        "JAX_COMPILATION_CACHE_DIR",
        os.path.expanduser("~/.cache/expdis_jax_compilation_cache"),
    )
    ...
    try:
        ...
        print(f"[expdis-jax] WARNING: could not enable persistent compile cache: {_e}", flush=True)
...
from .data import DapoExample, load_examples
...
from .lr_schedules import lr_at_update, stage_lr_fields, validate_lr_schedule
...
_BATCH_SHARD_GENERATION = 0
_BATCH_SHARD_PUBLISHED_GENERATION = 0
...
class _BatchShardHandler(http.server.BaseHTTPRequestHandler):
    def do_GET(self):  # noqa: N802
        try:
            prefix, generation, rank_text = self.path.strip("/").split("/")
            rank, generation = int(rank_text), int(generation)
            with _BATCH_SHARD_LOCK:
                if prefix != "batch" or generation != _BATCH_SHARD_PUBLISHED_GENERATION:
                    raise KeyError("batch generation is not ready")
        ...
        with _BATCH_SHARD_LOCK:
            if generation == _BATCH_SHARD_PUBLISHED_GENERATION:
...
def _coordinator_host() -> str:
    batch_source = (os.environ.get("EXPDIS_BATCH_SHARD_SOURCE_HOST") or "").strip()
    if batch_source:
...
def _local_routable_host() -> str:
    for key in ("EXPDIS_TPU_WORKER_IP", "EXPDIS_LOCAL_HOST_IP"):
...
def _batch_shard_source_host() -> str:
    ...
    """
    explicit = (os.environ.get("EXPDIS_BATCH_SHARD_SOURCE_HOST") or "").strip()
    if explicit:
...
def _batch_shard_log(message: str) -> None:
    if os.environ.get("EXPDIS_BATCH_SHARD_DEBUG", "1") in ("0", "false", "False"):
        return
    try:
        print(
...
def _stable_wandb_run_id(cfg: TrainConfig, metric_prefix: str) -> str:
    ...
    env_by_prefix = {
        "explorer_grpo": "EXPDIS_WANDB_EXPLORER_RUN_ID",
        "actual_grpo": "EXPDIS_WANDB_ACTUAL_GRPO_RUN_ID",
    }
    keys = []
    if metric_prefix == "actual_grpo":
        keys.append("EXPDIS_WANDB_ACTUAL_RUN_ID")
    keys.extend([env_by_prefix.get(metric_prefix, ""), "EXPDIS_WANDB_RUN_ID"])
    for key in keys:
...
def _wandb_log_kwargs(step: int, metric_prefix: str | None = None) -> dict:
    ...
    if _env_flag("EXPDIS_WANDB_SINGLE_RUN", "1"):
        if not _env_flag("EXPDIS_WANDB_EXPLICIT_STEP", "0"):
            return {}
        offsets = {
            "explorer_grpo": 0,
...
def _make_qwen3_config(model_name: str) -> Qwen3Config:
    ...
    if model_type == "ministral3" or (text_cfg is not None and (
        ...
        return Qwen3Config(
            ...
            use_qk_norm=False,
            hf_weight_prefix="language_model.model" if text_cfg is not None else "model",
            ...
            llama_4_scaling_beta=float(rope.get("llama_4_scaling_beta", 0.0) or 0.0),
            dtype=jnp.bfloat16,
            param_dtype=jnp.float32,
        )
    if "qwen3" not in model_name_l and model_type != "qwen3":
        raise NotImplementedError(
            "expdis-jax training currently supports Qwen3 plus text-only Ministral3 checkpoints. "
    ...
    c = c_full
    rope = c.get("rope_parameters") or c.get("rope_scaling") or {}
    rope_type = str(rope.get("rope_type", rope.get("type", "default")))
    if rope_type not in ("default", "yarn"):
        raise ValueError(f"unsupported Qwen3 rope_type={rope_type!r}")
    return Qwen3Config(
        hidden_size=int(c["hidden_size"]),
        intermediate_size=int(c["intermediate_size"]),
        num_hidden_layers=int(c["num_hidden_layers"]),
        ...
        rope_original_max_position_embeddings=int(rope.get("original_max_position_embeddings", c.get("max_position_embeddings", 40960))),
        rope_truncate=bool(rope.get("truncate", True)),
...
def _lr_schedule(cfg: TrainConfig, peak_lr: float, *, stage: str = "grpo"):
    """Learning rate (float or optax schedule) for one training stage.

    "constant" returns the plain float — byte-identical to the frozen
    contracts. Schedules are pure functions of the optax update count
    (successful optimizer updates only; gradient accumulation and discarded
    rollout groups never advance it), so crash-resume restores the exact
    position from the checkpointed opt_state.
    """
    ...
    if kind == "linear_warmup_constant":
        def _dapo_warmup_constant(count):
            # optax passes count = completed updates (0-based); update n = count+1.
            return peak * jnp.minimum((count + 1.0) / float(warmup), 1.0)
        return _dapo_warmup_constant
    floor = min_ratio * peak
    return optax.warmup_cosine_decay_schedule(
        init_value=0.0,
        peak_value=peak,
        warmup_steps=warmup if kind == "warmup_cosine" else 0,
        decay_steps=decay,
        end_value=floor,
    )


def _adamw_tx(cfg: TrainConfig, learning_rate: float, *, stage: str = "grpo"):
    ...
    return optax.chain(
        ...
        optax.adamw(
            learning_rate=_lr_schedule(cfg, learning_rate, stage=stage),
            b1=cfg.adam_beta1,
            b2=cfg.adam_beta2,
...
_CHECKPOINT_NONE = "none"
_CHECKPOINT_WEIGHTS = "weights"
_CHECKPOINT_RESUME = "resume"
_CHECKPOINT_LEGACY = "legacy"


def _checkpoint_request(cfg: TrainConfig) -> Tuple[str, str]:
    """Resolve the mutually exclusive checkpoint modes for a training stage.

    A weights handoff starts a *new* optimization stage from checkpoint params.
    An exact resume continues an interrupted stage, including optimizer moments
    and its logical/update step. ``init_checkpoint`` is retained only as a
    compatibility path for older launchers; new code must choose one of the two
    explicit fields so a missing optimizer state cannot silently turn a resume
    into a fresh stage.
    """
    weights = str(getattr(cfg, "init_weights_checkpoint", "") or "").strip()
    ...
    if len(selected) > 1:
        ...
        raise ValueError(
            "checkpoint sources are mutually exclusive; choose exactly one of "
            "init_weights_checkpoint (fresh stage) or resume_checkpoint "
            f"(exact resume), not: {names}"
        )
    return selected[0] if selected else (_CHECKPOINT_NONE, "")


def _checkpoint_metadata_keys(metadata: Any) -> set[str]:
    """Extract top-level Orbax keys without depending on one Orbax version."""
    candidates = [metadata]
    item_metadata = getattr(metadata, "item_metadata", None)
    if item_metadata is not None:
        candidates.append(item_metadata)
    ...
    for candidate in candidates:
        ...
        keys = getattr(candidate, "keys", None)
        if callable(keys):
            return {str(key) for key in keys()}
    return set()


def _require_exact_resume_metadata(checkpointer, restore_path: str) -> None:
    """Fail closed unless an exact-resume checkpoint proves it is complete."""
    try:
        metadata = checkpointer.metadata(restore_path)
    except Exception as exc:
        raise RuntimeError(
            f"cannot verify exact-resume checkpoint metadata: {restore_path}"
        ) from exc
    keys = _checkpoint_metadata_keys(metadata)
    required = {"params", "opt_state", "step"}
    missing = sorted(required - keys)
    if missing:
        raise RuntimeError(
            "exact resume requires params, opt_state, and step; checkpoint "
            f"{restore_path!r} is missing: {', '.join(missing)}"
        )


def _restore_params_only_from_checkpoint(params, param_sh, checkpoint: str, mesh):
    """Restore only model params for a fresh stage (no optimizer or step)."""
    if not str(checkpoint or "").strip():
        return params
    import orbax.checkpoint as ocp
    import orbax.checkpoint.checkpoint_utils as ocp_utils

    restore_path = _orbax_checkpoint_path(checkpoint)
    restore_item = {"params": params}
    ...
    if not isinstance(ckpt, Mapping) or ckpt.get("params") is None:
        raise RuntimeError(f"weights checkpoint missing params: {checkpoint}")
    restored = ckpt["params"]
    if jax.tree_util.tree_structure(restored) != jax.tree_util.tree_structure(params):
        raise RuntimeError(f"weights checkpoint parameter tree is incompatible: {checkpoint}")
    return jax.tree_util.tree_map(
        lambda value, sharding: _restore_param_on_mesh(value, sharding, mesh),
        restored,
        param_sh,
    )


def _fresh_state_with_params(state, params):
    """Install params while resetting optimizer state and the local stage step."""
    opt_state = state.tx.init(params)
    zero_step = jnp.zeros_like(state.step)
    if hasattr(state.step, "sharding"):
        zero_step = jax.device_put(zero_step, state.step.sharding)
    return state.replace(params=params, opt_state=opt_state, step=zero_step)


def _init_state(cfg: TrainConfig, mesh):
    # Fail closed BEFORE any model/optimizer work: a schedule position lives in
    # the optax update count inside opt_state, so resuming a non-constant
    # schedule without checkpointed opt_state would silently restart the warmup.
    _sched_kind, _sched_warmup, _sched_decay, _ = stage_lr_fields(cfg, "grpo")
    validate_lr_schedule(_sched_kind, _sched_warmup, _sched_decay, stage="grpo")
    if _sched_kind != "constant":
        _save_opt = os.environ.get("EXPDIS_SAVE_OPT_STATE", "0") not in ("0", "false", "False")
        if not _save_opt and os.environ.get(
            "EXPDIS_ALLOW_UNCHECKPOINTED_LR_SCHEDULE", "0"
        ) in ("0", "false", "False"):
            raise ValueError(
                "non-constant grpo LR schedule requires EXPDIS_SAVE_OPT_STATE=1 so the "
                "schedule position (optax update count) checkpoints and restores "
                "exactly on resume; set EXPDIS_ALLOW_UNCHECKPOINTED_LR_SCHEDULE=1 to override"
            )
    m_cfg = _make_qwen3_config(cfg.model_name)
    from .model import set_active_mesh
    set_active_mesh(mesh)
    model = Qwen3Model(m_cfg)
    _, checkpoint_path = _checkpoint_request(cfg)
    empty_for_restore = (
        bool(checkpoint_path)
        and os.environ.get("EXPDIS_INIT_EMPTY_FOR_RESTORE", "0") not in ("0", "false", "False")
    )
    ...
    tx = _adamw_tx(cfg, cfg.grpo_lr, stage="grpo")
...
def _orbax_checkpoint_path(path: str) -> str:
    ...
    if path.startswith("hf://"):
        local_override = os.environ.get("EXPDIS_INIT_CHECKPOINT_LOCAL_PATH", "").strip()
        if local_override:
            local_path = os.path.abspath(local_override)
            if not os.path.exists(local_path):
                raise FileNotFoundError(
                    "EXPDIS_INIT_CHECKPOINT_LOCAL_PATH is set but does not exist: "
                    f"{local_path!r}"
                )
...
def _maybe_restore_state_from_checkpoint(state, param_sh, cfg: TrainConfig, mesh):
    mode, checkpoint = _checkpoint_request(cfg)
    if mode == _CHECKPOINT_NONE:
        return state, 0
    import orbax.checkpoint as ocp
    import orbax.checkpoint.checkpoint_utils as ocp_utils

    print(f"[ckpt] resolving {mode} checkpoint: {checkpoint}", flush=True)
    restore_path = _orbax_checkpoint_path(checkpoint)
    print(f"[ckpt] restore path resolved: {restore_path}", flush=True)
    if mode == _CHECKPOINT_WEIGHTS:
        params = _restore_params_only_from_checkpoint(
            state.params, param_sh, checkpoint, mesh
        )
        state = _fresh_state_with_params(state, params)
        print(
            f"[train] initialized fresh stage from weights: {checkpoint} "
            "(optimizer=fresh, step=0)",
            flush=True,
        )
        return state, 0

    if mode == _CHECKPOINT_LEGACY:
        print(
            "[ckpt] WARNING: init_checkpoint uses legacy ambiguous restore "
            "semantics; migrate to init_weights_checkpoint or resume_checkpoint",
            flush=True,
        )
    ...
    if mode == _CHECKPOINT_RESUME:
        restore_item["opt_state"] = state.opt_state
    print("[ckpt] constructing restore args", flush=True)
    restore_args = ocp_utils.construct_restore_args(restore_item, strict=False)
    print("[ckpt] starting Orbax restore", flush=True)
    checkpointer = ocp.PyTreeCheckpointer()
    if mode == _CHECKPOINT_RESUME:
        _require_exact_resume_metadata(checkpointer, restore_path)
    ckpt = checkpointer.restore(
        restore_path,
        item=restore_item,
    ...
    if params is None:
        raise RuntimeError(f"Checkpoint missing params: {checkpoint}")
    ...
    restore_opt_state = mode == _CHECKPOINT_RESUME or os.environ.get(
        "EXPDIS_RESTORE_OPT_STATE", "1"
    ) not in (
        "0",
        "false",
        "False",
    ...
    if opt_state is not None and restore_opt_state:
        try:
            if (
                ...
                raise ValueError("optimizer-state tree does not match current optimizer")
            ...
            if mode == _CHECKPOINT_RESUME:
                raise RuntimeError(
                    f"exact-resume optimizer state is incompatible: {checkpoint}"
                ) from e
            print(
        ...
        print("[ckpt] skipped optimizer-state restore (EXPDIS_RESTORE_OPT_STATE=0)", flush=True)
    elif mode == _CHECKPOINT_RESUME:
        # Metadata should catch this first; retain a payload-level guard for
        # mocked/older checkpointers whose metadata surface is incomplete.
        raise RuntimeError(f"exact-resume checkpoint missing opt_state: {checkpoint}")
    restored_step = int(ckpt.get("step", 0) or 0)
    if restored_step < 0:
        raise RuntimeError(f"checkpoint has invalid negative step: {checkpoint}")
    if mode == _CHECKPOINT_RESUME:
        state_step = jnp.asarray(
            restored_step, dtype=getattr(state.step, "dtype", jnp.int32)
        )
        if hasattr(state.step, "sharding"):
            state_step = jax.device_put(state_step, state.step.sharding)
        state = state.replace(step=state_step)
    print(f"[train] restored {mode} checkpoint: {checkpoint} step={restored_step}", flush=True)
    return state, restored_step


def _maybe_restore_rnd_from_checkpoint(rnd_map, cfg: TrainConfig):
    mode, checkpoint = _checkpoint_request(cfg)
    if mode in (_CHECKPOINT_NONE, _CHECKPOINT_WEIGHTS) or rnd_map is None:
        return rnd_map
    ...
    reset_rnd = os.environ.get("EXPDIS_MULTIROUND_RESET_RND", "0").lower() in (
        "1", "true", "yes", "on"
    )
    if reset_rnd and mode == _CHECKPOINT_RESUME:
        raise ValueError(
            "exact resume cannot reset RND state; use init_weights_checkpoint "
            "for a fresh cross-round stage"
        )
    if reset_rnd:
        print("[novelty] EXPDIS_MULTIROUND_RESET_RND=1 — skipping RND restore for fresh per-round signal", flush=True)
        return rnd_map
    try:
        import orbax.checkpoint as ocp
        import orbax.checkpoint.checkpoint_utils as ocp_utils
        ...
        ckpt = ocp.PyTreeCheckpointer().restore(
            _orbax_checkpoint_path(checkpoint),
            item=restore_item,
            restore_args=ocp_utils.construct_restore_args(restore_item, strict=False),
            partial_restore=True,
        )
        ...
        if restored:
            print(f"[novelty] restored RND state from {checkpoint}", flush=True)
            return restore_rnd_map(rnd_map, restored)
        if mode == _CHECKPOINT_RESUME:
            raise RuntimeError(
                f"exact-resume novelty checkpoint missing rnd_state: {checkpoint}"
            )
    except Exception as e:
        if mode == _CHECKPOINT_RESUME:
            raise RuntimeError(
                f"exact-resume RND restore failed: {checkpoint}"
            ) from e
...
def _encode_novelty_texts(tokenizer, texts: List[str], cfg: TrainConfig, token_ids=None) -> Tuple[np.ndarray, np.ndarray, Dict[str, float]]:
    ...
    trunc_mode = (os.environ.get("EXPDIS_NOVELTY_TRUNCATION_MODE") or "tokenizer").lower()
    if token_ids is not None or trunc_mode == "head_tail":
        ...
        for index, text in enumerate(texts):
            ids = list(token_ids[index]) if token_ids is not None else tokenizer.encode(text, add_special_tokens=False)
            original_lengths.append(int(len(ids)))
            if len(ids) > max_len and trunc_mode == "head_tail":
                head = max_len // 2
                tail = max_len - head
                ids = ids[:head] + ids[-tail:]
            encoded_rows.append([int(x) for x in ids])
        width = sequence_bucket(min(max(original_lengths, default=0), max_len), max_len)
        input_ids = np.full((len(encoded_rows), width), pad_id, dtype=np.int32)
...
def _broadcast_novelty_feature_inputs(
    *,
    ...
    debug = _env_flag("EXPDIS_NOVELTY_BROADCAST_DEBUG", "0")
...
def _extract_novelty_features_from_encoded(
    *,
    ...
    batch_size = max(1, int(os.environ.get("EXPDIS_NOVELTY_BATCH_SIZE", "1") or "1"))
    total_rows = int(input_ids.shape[0])
    progress_every = max(batch_size, int(os.environ.get("EXPDIS_NOVELTY_PROGRESS_EVERY_ROWS", "32") or "32"))
    ...
    for start in range(0, input_ids.shape[0], batch_size):
        ...
        # Keep left padding and every active position; trim trailing padding only.
        active_columns = np.flatnonzero(np.any(mask, axis=0))
        last_active = int(active_columns[-1]) + 1 if active_columns.size else 0
        width = sequence_bucket(last_active, ids.shape[1])
...
def _extract_novelty_features(
    *,
    ...
    input_ids, attention_mask, stats = _encode_novelty_texts(tokenizer, texts, cfg, token_ids=token_ids)
...
def _completion_token_ids(tokenizer, completion, max_length: int) -> list[int]:
    """Use sampled actions; tokenize once only for offline/legacy completions."""
    sampled = getattr(completion, "generated_token_ids", None)
    if sampled is not None:
        return list(sampled[:int(max_length)])
    cached = getattr(completion, "training_token_ids", None)
    if cached is None:
        cached = list(tokenizer(
            getattr(completion, "text", "") or "",
            add_special_tokens=False,
        )["input_ids"])
        completion.training_token_ids = cached
    return cached[:int(max_length)]


def _update_rnd_after_policy(rnd_map, feature_batches, cfg):
    """One predictor update on the selected rows' pre-policy-update features.

    Scoring all dynamic-sampling attempts uses the same frozen predictor. The
    features are retained on the host, so this never reruns the changed policy.
    """
    if not feature_batches or rnd_map is None:
        return rnd_map
    selected = {}
    for rows, features in feature_batches:
        indices = [i for i, row in enumerate(rows) if row.get("used_for_training", True)]
        if indices:
            for key, values in features.items():
                selected.setdefault(key, []).append(np.asarray(values)[indices])
    if not selected:
        return rnd_map
    features = {key: np.concatenate(values, axis=0) for key, values in selected.items()}
    kwargs = dict(lr=float(cfg.rnd_lr), metric=cfg.novelty_metric,
                  clip_value=float(cfg.novelty_clip or 0.0), train=True)
    updated, _ = score_and_update_rnd_map(
        rnd_map, features, hidden_dim=int(cfg.rnd_hidden), **kwargs)
    return updated
...
def _score_rollouts(
    *,
    ...
    rnd_feature_batches: Optional[list] = None,
    ...
    progress_every = max(1, int(os.environ.get("EXPDIS_SCORE_PROGRESS_EVERY_ROWS", "1") or "1"))
    scored_count = 0

    for ex, comps in zip(examples, completions):
        for c in comps:
            completion_text = getattr(c, "text", "") or ""
            scoring_text = completion_text
            finish_reason = str(getattr(c, "finish_reason", "") or "").lower()
            token_ids = _completion_token_ids(tokenizer, c, cfg.max_completion_len)
            completion_token_length = len(token_ids)
            if finish_reason == "length":
                completion_token_length = int(cfg.max_completion_len)
            clipped = finish_reason == "length" or completion_token_length >= int(cfg.max_completion_len)
            terminated = not clipped and finish_reason in ("stop", "eos", "eos_token", "stop_sequence")
    ...
    if float(cfg.lambda_novelty or 0.0) != 0.0:
        ...
        features_by_layer, feature_stats = _extract_novelty_features(
            ...
            feature_step=feature_step,
            token_ids=[_completion_token_ids(tokenizer, c, cfg.max_completion_len)
                       for group in completions for c in group]
            if cfg.novelty_text_contract == "full_raw" else None,
        )
        ...
        print("[novelty] RND score complete", flush=True)
        if rnd_feature_batches is not None:
            rnd_feature_batches.append((rows, {
                key: np.asarray(value, dtype=np.float32)
                for key, value in features_by_layer.items()
            }))
        if per_layer:
...
def _build_rollout_batch(tokenizer, examples: List[DapoExample], completions, cfg: TrainConfig, scored_rows: Optional[List[Dict[str, Any]]] = None) -> Dict[str, Any]:
    ...
    for ex, comps in zip(examples, completions):
        # Keep the *tail* of an over-long prompt so the chat template's assistant
        # header (which the served model saw) stays adjacent to the completion.
        prompt_ids = tokenizer(ex.prompt_text, add_special_tokens=False)["input_ids"][-P:]
        left_pad = P - len(prompt_ids)
        prompt_row = [pad_id] * left_pad + prompt_ids
        prompt_attn = [0] * left_pad + [1] * len(prompt_ids)
        for c in comps:
            comp_ids = _completion_token_ids(tokenizer, c, T)
            comp_pad = comp_ids + [pad_id] * (T - len(comp_ids))
            rows_full_ids.append(prompt_row + comp_pad)
            rows_attn.append(prompt_attn + [1] * len(comp_ids) + [0] * (T - len(comp_ids)))
            ...
            row["completion_token_length"] = int(row.get("completion_token_length", len(comp_ids)))
            row["clipped"] = bool(row.get("clipped", False)) or len(comp_ids) >= int(T)
...
def _dynamic_sample_rollouts(
    *,
    ...
    groups_per_attempt = max(
        1,
        int(os.environ.get("EXPDIS_DYNAMIC_SAMPLING_GROUPS_PER_ATTEMPT", "1") or "1"),
    )
    ...
    while attempt < max_attempts and candidate_groups < max_candidate_groups:
        ...
        attempt_completions = batched_generate(
            [tokenizer(p.prompt_text, add_special_tokens=False)["input_ids"][-cfg.max_prompt_len:] for p in attempt_prompts],
            server_urls,
            ...
            seed_base=int(rng.integers(0, 2_147_483_647)),
        )
        rollout_dt += time.perf_counter() - t0
        if _env_flag("EXPDIS_DISABLE_ROLLOUT_DIVERSITY", "0"):
        ...
        scored_rows, rnd_map, _attempt_summary = _score_rollouts(
            ...
            rnd_feature_batches=rnd_feature_batches,
        )
        ...
        for group_index, (ex, comps, group_rows) in enumerate(zip(attempt_prompts, attempt_completions, grouped_rows)):
            ...
            eligibility_mode = (
                os.environ.get("EXPDIS_DYNAMIC_SAMPLING_ELIGIBILITY", "blended_reward")
                or "blended_reward"
            ).strip().lower()
            if eligibility_mode == "novelty_contribution":
                # GPU c8 parity (paper Appendix B): eligibility variance comes
                # only from the correctness-gated novelty contribution λ·r̃.
                # Correctness and soft-overlong shaping are excluded, so an
                # all-incorrect group can never qualify (novelty is credited
                # only on correct completions) and an all-correct group
                # qualifies only through genuine novelty variance.
                reward_values = np.asarray(
                    [
                        float(cfg.lambda_novelty) * float(row.get("novelty_reward", 0.0) or 0.0)
            ...
            # Paper protocol: a group is informative when its total per-trajectory
            # reward (correctness + gated novelty + soft-overlong shaping) has
            # variance, in the Explorer and the Main-model stage alike.  Groups
            # whose members all earn the same total reward (typically all-correct
            # or all-incorrect) carry no signal and are resampled.
            accept_reward_variance = _env_flag("EXPDIS_DYNAMIC_SAMPLING_ACCEPT_REWARD_VARIANCE", "1")
            reward_informative = accept_reward_variance and reward_std > float(
                os.environ.get("EXPDIS_DYNAMIC_SAMPLING_MIN_REWARD_STD", "1e-6") or "1e-6"
            )
            if correct_count == 0:
    ...
    # Average over the attempts that actually accumulated diversity entries
    # (one per generation attempt), not over candidate groups.
    diversity_metrics = {k: v / float(max(1, diversity_attempts)) for k, v in diversity_acc.items()}
...
def _trim_rollout_batch_tokens(batch: Dict[str, Any], prompt_len: int, multiple: int = 256) -> Dict[str, Any]:
    ...
    The training contract fixes a completion *cap* (32,768 tokens), not a
    requirement to run every update at the full cap. JAX attention still pays for padded sequence columns, so on
    ...
    """
    if os.environ.get("EXPDIS_TRIM_ROLLOUT_BATCH_TOKENS", "1") in ("0", "false", "False"):
        return batch
    ...
    min_completion = int(os.environ.get("EXPDIS_MIN_TRAIN_COMPLETION_COLUMNS", "512") or "512")
    used = max(1, used, min_completion)
    multiple = max(1, int(os.environ.get("EXPDIS_TRIM_TOKEN_MULTIPLE", str(multiple)) or str(multiple)))
...
def _shard_batch_for_hosts(batch: Dict[str, Any], pspecs: Dict[str, P]) -> Dict[str, Any]:
    ...
    """
    global _BATCH_SHARD_GENERATION, _BATCH_SHARD_PUBLISHED_GENERATION
    # Every rank enters this function once per batch (or microbatch). Keep the
    # counter across stages so an early peer can never consume the last stage's
    # still-published payload. Publication and acknowledgment are atomic.
    _BATCH_SHARD_GENERATION += 1
    generation = _BATCH_SHARD_GENERATION
    if os.environ.get("EXPDIS_REPLICATE_ROLLOUT_BATCH", "0") not in ("0", "false", "False"):
        ...
        port = int(os.environ.get("EXPDIS_BATCH_SHARD_HTTP_PORT", "18777"))
        ...
        if is_source:
            ...
            with _BATCH_SHARD_LOCK:
                ...
                _BATCH_SHARD_PUBLISHED_GENERATION = generation
        ...
        if is_source:
            ...
            source_wait_s = float(os.environ.get("EXPDIS_BATCH_SHARD_SOURCE_WAIT_SECONDS", "90"))
        ...
        url = f"http://{source_host}:{port}/batch/{generation}/{int(jax.process_index())}"
        timeout = float(os.environ.get("EXPDIS_BATCH_SHARD_HTTP_TIMEOUT", "120"))
        attempts = int(os.environ.get("EXPDIS_BATCH_SHARD_HTTP_ATTEMPTS", "240"))
    ...
    if jax.process_count() <= 1 or os.environ.get("EXPDIS_BATCH_SHARD_HTTP", "1") in ("0", "false", "False"):
        if jax.process_count() > 1:
            # broadcast_one_to_all requires IDENTICAL array shapes on all hosts,
            # but the source host's batch is token-trimmed (and row-sliced under
            # host grad accum) while peers hold untrimmed placeholders. The HTTP
            # shard path handles this; the raw broadcast fallback cannot.
            raise RuntimeError(
                "EXPDIS_BATCH_SHARD_HTTP=0 is unsupported on multihost runs: the "
                "source host's rollout batch is shape-trimmed and would not "
                "match peer shapes in broadcast_one_to_all. Unset "
                "EXPDIS_BATCH_SHARD_HTTP (default 1) to use the HTTP shard path."
            )
        ...
        for rank in range(process_count):
            ...
            for key, value in batch.items():
                ...
                if pspec is not None and first_axis == FSDP_AXIS and arr.ndim > 0 and arr.shape[0] == rows:
                    ...
                    shard[key] = arr
            shards.append(shard)
        serialized = {rank: pickle.dumps(shard, protocol=pickle.HIGHEST_PROTOCOL) for rank, shard in enumerate(shards)}
        with _BATCH_SHARD_LOCK:
            _BATCH_SHARD_PAYLOADS.clear()
            _BATCH_SHARD_SERVED_RANKS.clear()
            _BATCH_SHARD_PAYLOADS.update(serialized)
            _BATCH_SHARD_PUBLISHED_GENERATION = generation
        _ensure_batch_shard_server(port)
        _batch_shard_log(
            "source ready "
...
def make_micro_grad_step(
    ...
    cap-wide batch shape when only a small subset of rows are clipped.
    """
...
def _trajectory_gcs_dir(cfg: TrainConfig, *, run_name: Optional[str] = None) -> str:
    gcs_base = os.environ.get("EXPDIS_GCS_CHECKPOINT_BASE", "").strip()
    if not gcs_base:
...
def _vllm_timeout_seconds() -> int:
    return max(1, _env_int("EXPDIS_VLLM_REQUEST_TIMEOUT_SECONDS", 180))


def _vllm_max_retries() -> int:
    return max(0, _env_int("EXPDIS_VLLM_MAX_RETRIES", 0))


def _upload_trajectory_file(cfg: TrainConfig, local_path: str, target_names: Tuple[str, ...]) -> None:
    if not os.path.exists(local_path):
        return
    if not _env_flag("EXPDIS_ALLOW_GCS_ARTIFACTS", "0"):
        if _env_flag("EXPDIS_HF_MIRROR_TRAJECTORIES", "0"):
...
def _base_policy_snapshot_enabled(cfg: TrainConfig) -> bool:
    mode, _ = _checkpoint_request(cfg)
    if mode != _CHECKPOINT_NONE:
        return False
    return os.environ.get("EXPDIS_SAVE_BASE_POLICY_TRAJECTORIES", "1") not in (
        "0",
        "false",
        "False",
        "no",
...
def _save_base_policy_trajectory_snapshot(
    *,
    ...
    if not _base_policy_snapshot_enabled(cfg):
        ...
        if mode != _CHECKPOINT_NONE:
            print(
                f"[base-policy] skipped: {mode} checkpoint is set, so this is not a base-policy run",
                flush=True,
            )
        return
    if not examples:
    ...
    batches = max(1, int(os.environ.get("EXPDIS_BASE_POLICY_TRAJECTORY_BATCHES", "1") or "1"))
    seed = int(os.environ.get("EXPDIS_BASE_POLICY_TRAJECTORY_SEED", str(cfg.seed)) or str(cfg.seed))
    ...
    base_max_tokens = int(
        os.environ.get("EXPDIS_BASE_POLICY_MAX_COMPLETION_LEN", str(cfg.max_completion_len))
    ...
    for batch_idx in range(batches):
        ...
        completions = batched_generate(
            [tokenizer(p.prompt_text, add_special_tokens=False)["input_ids"][-cfg.max_prompt_len:] for p in prompts],
            server_urls,
            n_per_prompt=cfg.grpo_num_generations,
...
from expdis_jax.weights import export_flax_params_to_hf_dir

ckpt_path = os.environ["EXPDIS_EXPORT_ORBAX_CKPT"]
output_dir = os.environ["EXPDIS_EXPORT_HF_OUTPUT_DIR"]
model_name = os.environ["EXPDIS_EXPORT_MODEL_NAME"]
num_layers = int(os.environ["EXPDIS_EXPORT_NUM_HIDDEN_LAYERS"])
tie_word_embeddings = os.environ.get("EXPDIS_EXPORT_TIE_WORD_EMBEDDINGS", "1") not in ("0", "false", "False")
use_qk_norm = os.environ.get("EXPDIS_EXPORT_USE_QK_NORM", "1") not in ("0", "false", "False")
hf_weight_prefix = os.environ.get("EXPDIS_EXPORT_HF_WEIGHT_PREFIX", "model")
    ...
    env.update(
        {
            ...
            "EXPDIS_EXPORT_ORBAX_CKPT": gcs_checkpoint_path,
            "EXPDIS_EXPORT_HF_OUTPUT_DIR": os.path.abspath(local_dir),
            "EXPDIS_EXPORT_MODEL_NAME": cfg.model_name,
            "EXPDIS_EXPORT_NUM_HIDDEN_LAYERS": str(int(m_cfg.num_hidden_layers)),
            "EXPDIS_EXPORT_TIE_WORD_EMBEDDINGS": "1" if bool(m_cfg.tie_word_embeddings) else "0",
            "EXPDIS_EXPORT_USE_QK_NORM": "1" if bool(m_cfg.use_qk_norm) else "0",
            "EXPDIS_EXPORT_HF_WEIGHT_PREFIX": str(m_cfg.hf_weight_prefix),
        }
    )
    timeout = float(os.environ.get("EXPDIS_VLLM_EXPORT_FROM_CKPT_TIMEOUT_SECONDS", "7200") or "7200")
    ret = subprocess.run(
...
def _maybe_reload_vllm_from_gcs(
    ...
    *,
    ...
    env.update(
        {
            ...
            "MAX_NUM_SEQS": os.environ.get("EXPDIS_TRAIN_RELOAD_VLLM_MAX_NUM_SEQS", "32"),
    ...
    try:
        ret = subprocess.run(
            ["bash", os.path.join(os.path.dirname(os.path.abspath(__file__)), "scripts", "reload_vllm_direct_slice.sh")],
            capture_output=True,
            text=True,
            timeout=7200,
            env=env,
...
def _internal_reload_host() -> str:
    explicit = (os.environ.get("EXPDIS_VLLM_INTERNAL_RELOAD_HOST") or "").strip()
    if explicit:
        return explicit
    coord = (os.environ.get("JAX_COORDINATOR_ADDRESS") or "").strip()
    if coord:
...
def _start_internal_reload_tar_server(local_dir: str, cfg: TrainConfig, step: int) -> tuple[str, subprocess.Popen] | tuple[str, None]:
    ...
    """
    if os.environ.get("EXPDIS_VLLM_RELOAD_TRANSPORT", "hf").strip().lower() not in (
        "internal_http",
        "internal-tar",
        "pod_http",
    ):
    ...
    if not host:
        raise RuntimeError("EXPDIS_VLLM_RELOAD_TRANSPORT=internal_http requires EXPDIS_VLLM_INTERNAL_RELOAD_HOST or JAX_COORDINATOR_ADDRESS")
    ...
    port = int(os.environ.get("EXPDIS_VLLM_INTERNAL_RELOAD_PORT", "18765") or "18765")
...
def _request_external_vllm_reload(
    ...
    *,
    ...
    payload = {
        ...
        "tpu_name": os.environ.get("EXPDIS_VLLM_RELOAD_TPU_NAME", ""),
        "tpu_project": (
            os.environ.get("EXPDIS_VLLM_RELOAD_TPU_PROJECT")
            or os.environ.get("TPU_PROJECT")
            or os.environ.get("CLOUDSDK_CORE_PROJECT")
        ...
        "zone": os.environ.get("EXPDIS_VLLM_RELOAD_ZONE", ""),
        "workers": os.environ.get("EXPDIS_VLLM_RELOAD_WORKERS", ""),
        "server_urls": os.environ.get("EXPDIS_VLLM_RELOAD_SERVER_URLS", cfg.vllm_server_urls),
        "multi_server": os.environ.get("EXPDIS_VLLM_MULTI_SERVER", "0"),
        "chips": os.environ.get("EXPDIS_VLLM_MULTI_SERVER_CHIPS", ""),
        "port_base": os.environ.get("EXPDIS_VLLM_MULTI_SERVER_PORT_BASE", ""),
    ...
    if os.environ.get("EXPDIS_EXTERNAL_VLLM_RELOAD_WAIT", "1") in ("0", "false", "False"):
        return True
    ...
    required = os.environ.get("EXPDIS_EXTERNAL_VLLM_RELOAD_REQUIRED", "1") not in ("0", "false", "False")
    retry_on_fail = os.environ.get("EXPDIS_EXTERNAL_VLLM_RELOAD_RETRY_ON_FAIL", "1") not in ("0", "false", "False")
...
def _maybe_export_and_reload_vllm(state, cfg: TrainConfig, m_cfg: Qwen3Config, step: int, *, reason: str) -> None:
    live_allgather_export = (
        jax.process_count() > 1
        and _env_flag("EXPDIS_VLLM_EXPORT_FROM_LIVE_ALLGATHER", "1")
        and not _env_flag("EXPDIS_VLLM_EXPORT_FROM_GCS_CHECKPOINT", "0")
    )
    ...
    mirror_to_hf = _env_flag("EXPDIS_HF_MIRROR_VLLM_EXPORTS", "0") and bool(hf_repo_id)
    internal_reload_transport = os.environ.get("EXPDIS_VLLM_RELOAD_TRANSPORT", "hf").strip().lower() in (
        "internal_http",
        "internal-tar",
        "pod_http",
    )
    if not gcs_base and not mirror_to_hf and not internal_reload_transport:
        # The paper protocol synchronizes the sampler before every rollout
        # batch, so a missing transport is a configuration error unless the
        # caller explicitly opts out (EXPDIS_EXTERNAL_VLLM_RELOAD_REQUIRED=0,
        # e.g. for a smoke run that tolerates stale rollouts).
        message = (
            "[vllm-reload] no GCS base, HF artifact repo (EXPDIS_HF_MIRROR_VLLM_EXPORTS=1), or "
            "internal transport is configured, so the serving copy cannot be synchronized to the "
            f"trainer ({reason}); rollouts would keep sampling from the previously served weights"
        )
        if os.environ.get("EXPDIS_EXTERNAL_VLLM_RELOAD_REQUIRED", "1") not in ("0", "false", "False"):
            raise RuntimeError(message + "; set EXPDIS_EXTERNAL_VLLM_RELOAD_REQUIRED=0 to run anyway")
        if jax.process_index() == 0:
            print(message + " (WARN: continuing because EXPDIS_EXTERNAL_VLLM_RELOAD_REQUIRED=0)", flush=True)
        return
    ...
    reload_required = os.environ.get("EXPDIS_EXTERNAL_VLLM_RELOAD_REQUIRED", "1") not in ("0", "false", "False")
    reload_mode = os.environ.get("EXPDIS_TRAIN_VLLM_RELOAD_MODE", "direct").strip().lower()
    external_wait = os.environ.get("EXPDIS_EXTERNAL_VLLM_RELOAD_WAIT", "1") not in ("0", "false", "False")
    waits_for_reload = reload_mode != "external" or external_wait or reload_required
    async_hf_upload = (
        os.environ.get("EXPDIS_HF_MIRROR_ASYNC", "1") not in ("0", "false", "False", "no")
        and os.environ.get("EXPDIS_REQUIRE_HF_ARTIFACT_UPLOAD", "0") in ("0", "false", "False", "no")
    )
    delete_local = os.environ.get("EXPDIS_DELETE_LOCAL_VLLM_RELOAD_AFTER_UPLOAD", "0") not in (
        "0",
        "false",
        "False",
        "no",
    ...
    try:
        ...
        elif jax.process_count() > 1 and _env_flag("EXPDIS_VLLM_EXPORT_FROM_GCS_CHECKPOINT", "0"):
        ...
        gc.collect()
        if reload_required and not (internal_reload_url or uploaded or hf_uploaded):
            raise RuntimeError("vLLM synchronization failed: no reload bundle was delivered")
...
def _maybe_run_aime_probe(
    *,
    ...
    This is a trend metric; final AIME24 evaluation uses 30 problems and
    64 rollouts after Main RL.
    """
    every = int(os.environ.get("EXPDIS_AIME_PROBE_EVERY_STEPS", "0") or "0")
    ...
    max_tokens = int(os.environ.get("EXPDIS_AIME_PROBE_MAX_TOKENS", str(cfg.max_completion_len)) or str(cfg.max_completion_len))
    ...
    num_problems = int(os.environ.get("EXPDIS_AIME_PROBE_NUM_PROBLEMS", "6") or "6")
    num_rollouts = int(os.environ.get("EXPDIS_AIME_PROBE_NUM_ROLLOUTS", "4") or "4")
    temperature = float(os.environ.get("EXPDIS_AIME_PROBE_TEMPERATURE", "0.6") or "0.6")
    top_p = float(os.environ.get("EXPDIS_AIME_PROBE_TOP_P", str(cfg.top_p)) or str(cfg.top_p))
    top_k = int(os.environ.get("EXPDIS_AIME_PROBE_TOP_K", str(cfg.top_k)) or str(cfg.top_k))
    concurrency = int(os.environ.get("EXPDIS_AIME_PROBE_CONCURRENCY", str(min(8, max(1, cfg.vllm_concurrency)))) or "8")
    fanout = int(os.environ.get("EXPDIS_AIME_PROBE_FANOUT_PER_PROMPT", str(min(cfg.vllm_fanout_per_prompt, max(1, num_rollouts)))) or "1")
    ...
    try:
        from .eval import annotate_probe_trend, run_eval
        if use_wandb:
            import wandb

        def _progress(done: int, total: int, elapsed: float) -> None:
            if not use_wandb or not _env_flag("EXPDIS_WANDB_EVAL_PROGRESS", "0"):
                return
        ...
        if _env_flag("EXPDIS_HF_MIRROR_EVAL_PROBES", "0"):
        ...
        if os.environ.get("EXPDIS_AIME_PROBE_REQUIRED", "0") in ("1", "true", "True", "yes"):
            raise
...
def run_training(cfg: TrainConfig):
    ...
    if os.environ.get("EXPDIS_VLLM_SERVER_URLS"):
        if not cfg.vllm_server_urls:
            cfg = type(cfg)(**{**asdict(cfg), "vllm_server_urls": os.environ["EXPDIS_VLLM_SERVER_URLS"]})
    server_urls = parse_server_urls(cfg.vllm_server_urls)
    if not server_urls:
        raise RuntimeError("Set --vllm-server-urls or EXPDIS_VLLM_SERVER_URLS to vLLM /v1 endpoints.")

    os.makedirs(cfg.output_dir, exist_ok=True)
    traj_path = os.path.join(cfg.output_dir, "explorer_trajectories.jsonl")
    checkpoint_mode, _ = _checkpoint_request(cfg)
    if is_source_host and checkpoint_mode == _CHECKPOINT_RESUME and not os.path.exists(traj_path):
        raise RuntimeError(
            "exact resume requires the existing stage trajectory log; missing "
            f"{traj_path!r}"
        )
    if checkpoint_mode in (_CHECKPOINT_RESUME, _CHECKPOINT_LEGACY) and os.path.exists(traj_path):
        print(f"[train] resume mode: appending trajectories to {traj_path}", flush=True)
    elif is_source_host:
        open(traj_path, "w").close()  # truncate for base or fresh weights handoff
    ...
    if is_source_host:
        ...
        print(
            ...
            f"loss_normalizer={int(cfg.dr_grpo_loss_normalizer)} "
            ...
            f"reload_every={os.environ.get('EXPDIS_TRAIN_VLLM_RELOAD_EVERY_STEPS', '0')} "
    ...
    train_prompts_per_step = int(cfg.grpo_batch_size) * int(cfg.grpo_grad_accum)
    prompts_per_step = int(os.environ.get("EXPDIS_EFFECTIVE_PROMPTS_PER_STEP", str(train_prompts_per_step)) or train_prompts_per_step)
    if prompts_per_step <= 0:
        raise ValueError("effective prompts per step must be > 0")
    replicate_rollout_batch = os.environ.get("EXPDIS_REPLICATE_ROLLOUT_BATCH", "0") not in ("0", "false", "False")
    ...
    if cfg.trajectory_batch_size != prompts_per_step:
        print(
            f"[train] note: trajectory_batch_size={cfg.trajectory_batch_size} is unused here; "
            f"the contract prompts_per_step={prompts_per_step} governs rollout geometry",
            flush=True,
        )
    ...
    with mesh:
        ...
        if novelty_enabled:
            ...
            if (cfg.novelty_layer_agg or "mean").lower() != "mean":
                raise ValueError(
                    "JAX novelty implements only novelty_layer_agg='mean'; "
                    f"got {cfg.novelty_layer_agg!r} (the torch pilot's 'weighted' mode is not ported)"
                )
            layers = tuple(int(x) for x in cfg.novelty_layers)
            quarter_depth_layers = (
                int(m_cfg.num_hidden_layers) // 4,
                int(m_cfg.num_hidden_layers) // 2,
                (3 * int(m_cfg.num_hidden_layers)) // 4,
            )
            if not layers:
                layers = quarter_depth_layers
            elif layers != quarter_depth_layers and _env_flag("EXPDIS_DAPO_DRGRPO_FULL_CONTRACT", "1"):
                raise ValueError(
                    f"novelty_layers={list(layers)} must be the quarter-depth set "
                    f"{list(quarter_depth_layers)} for this model")
            for layer in layers:
        ...
        grpo_cfg = GrpoConfig(
            ...
            loss_normalizer=cfg.dr_grpo_loss_normalizer,
        ...
        if is_source_host:
            ...
            if int(os.environ.get("EXPDIS_DATASET_SHARD_COUNT", "1") or "1") > 1:
                    ...
                    json.dump(
                        {
                            ...
                            "shard_policy": os.environ.get("EXPDIS_DATASET_SHARD_POLICY", "uniform"),
                            "shard_seed": int(os.environ.get("EXPDIS_DATASET_SHARD_SEED", "0") or "0"),
        ...
        debug_compare_paths = os.environ.get("EXPDIS_DEBUG_COMPARE_LOGPROB_PATHS", "0") not in ("0", "false", "False")
        use_inline_old_logprobs = os.environ.get("EXPDIS_USE_INLINE_OLD_LOGPROBS", "1") not in ("0", "false", "False")
        ...
        track_external_old_gap = debug_compare_paths or (
            ...
            and os.environ.get("EXPDIS_TRACK_EXTERNAL_OLD_GAP", "0") not in ("0", "false", "False")
        )
        if os.environ.get("EXPDIS_VECTORIZE_GRPO_ROWS", "0") not in ("0", "false", "False"):
            row_vector_chunk = rows_per_microbatch
        else:
            row_vector_chunk = int(os.environ.get("EXPDIS_GRPO_ROW_VECTOR_CHUNK", "1"))
        ...
        replicate_rollout_batch = os.environ.get("EXPDIS_REPLICATE_ROLLOUT_BATCH", "0") not in (
            "0",
            "false",
            "False",
        )
        host_grad_accum = os.environ.get("EXPDIS_HOST_GRAD_ACCUM", "0") not in ("0", "false", "False")
        compact_nonzero_adv = os.environ.get("EXPDIS_HOST_GRAD_COMPACT_NONZERO_ADV", "0") not in (
            "0",
            "false",
            "False",
        )
        compact_adv_eps = float(os.environ.get("EXPDIS_HOST_GRAD_COMPACT_ADV_EPS", "1e-8"))
        compact_fixed_token_shape = os.environ.get("EXPDIS_HOST_GRAD_FIXED_TOKEN_SHAPE", "0") not in (
            "0",
            "false",
            "False",
        )
        if host_grad_accum:
            if updates_per_rollout != 1:
                raise ValueError("EXPDIS_HOST_GRAD_ACCUM supports only grpo_updates_per_rollout=1")
            if not use_inline_old_logprobs or track_external_old_gap:
                raise ValueError(
                    "EXPDIS_HOST_GRAD_ACCUM currently requires inline old logprobs "
                    "and EXPDIS_TRACK_EXTERNAL_OLD_GAP=0"
                )
            if ref_params is not None:
                raise ValueError("EXPDIS_HOST_GRAD_ACCUM currently requires KL/ref_params disabled")
            total_rows_per_step = (
            ...
            if total_rows_per_step % host_grad_rows_per_microbatch != 0:
                raise ValueError(
                    "EXPDIS_HOST_GRAD_ACCUM requires total rollout rows to divide host micro rows: "
        ...
        grpo_metric_prefix = (
            os.environ.get("EXPDIS_WANDB_GRPO_PREFIX")
        ...
        try:
            ...
            if is_source_host:
                ...
                wandb.init(
                    ...
                    id=wandb_run_id,
                    resume="allow",
                    group=os.environ.get("EXPDIS_WANDB_GROUP") or (cfg.wandb_run_name or f"jax_lambda{cfg.lambda_novelty}"),
        ...
        _maybe_export_and_reload_vllm(state, cfg, m_cfg, global_step, reason="stage_start")
        ...
        if global_step > 0 and is_source_host:
            print(f"[train] resumed at global_step={global_step}; prompt RNG is indexed by update", flush=True)
        ...
        # Paper protocol: the behavior policy is frozen per rollout batch, so the
        # serving copy is synchronized to the trainer after every optimizer
        # update, before the next rollout batch is sampled.  This cadence is
        # independent of the checkpoint/export cadence (save_every_steps).
        # Values > 1 are ablations; 0 disables refreshes (every rollout then
        # comes from the stage-start weights).
        reload_every_steps = _env_int("EXPDIS_TRAIN_VLLM_RELOAD_EVERY_STEPS", 1)
        if reload_every_steps != 1 and int(cfg.grpo_max_steps) > 1 and is_source_host:
            print(
                f"[vllm-reload] WARN EXPDIS_TRAIN_VLLM_RELOAD_EVERY_STEPS={reload_every_steps}: the paper "
                "protocol synchronizes the sampler before every rollout batch (1); "
                + ("the serving policy is never refreshed during this stage"
                   if reload_every_steps <= 0 else "rollouts will lag the trainer"),
                flush=True,
            )
        reload_first_step = _env_int("EXPDIS_TRAIN_VLLM_RELOAD_FIRST_STEP", 0)
        local_checkpoint_every_steps = _env_int("EXPDIS_LOCAL_CHECKPOINT_EVERY_STEPS", 0)
        gcs_checkpoint_every_steps = _env_int(
            "EXPDIS_GCS_CHECKPOINT_EVERY_STEPS",
            int(cfg.save_every_steps),
        )
        gcs_checkpoint_first_step = _env_int("EXPDIS_GCS_CHECKPOINT_FIRST_STEP", 0)
        ...
        prefetch_rollouts = (
            ...
            and os.environ.get("EXPDIS_PREFETCH_ROLLOUTS", "0") not in ("0", "false", "False")
        )
        ...
        def _start_rollout_prefetch() -> Optional[Future]:
            ...
            def _job():
                ...
                prefetched_completions = batched_generate(
                    [tokenizer(p.prompt_text, add_special_tokens=False)["input_ids"][-cfg.max_prompt_len:] for p in prefetched_prompts],
                    server_urls,
                    n_per_prompt=cfg.grpo_num_generations,
                    max_tokens=cfg.max_completion_len,
                    temperature=cfg.temperature,
        ...
        while global_step < cfg.grpo_max_steps:
            # Each paper-contract iteration owns its randomness. Variable
            # dynamic-sampling retries do not change the next update's stream,
            # including when resuming from a checkpoint.
            if cfg.dynamic_sampling:
                rng = np.random.default_rng(np.random.SeedSequence([int(cfg.seed), global_step]))
            # 1. Sample prompts
            ...
            for update_idx in range(updates_per_rollout):
                ...
                if host_grad_accum:
                    ...
                    micro_count = int(host_grad_accum_steps)
            ...
            if is_source_host:
                rnd_map = _update_rnd_after_policy(rnd_map, rnd_feature_batches, cfg)
            global_step += 1
            ...
            if use_wandb:
                ...
                compact_metrics = {
                    ...
                    "learning_rate": lr_at_update(cfg, cfg.grpo_lr, max(1, int(global_step * updates_per_rollout)), stage="grpo"),
                ...
                if _env_flag("EXPDIS_WANDB_MINIMAL", "1"):
                    wandb_metrics = {
                        key: value
                ...
                if cfg.dynamic_sampling and not _env_flag("EXPDIS_WANDB_MINIMAL", "1"):
                    payload.update({
                ...
                if not _env_flag("EXPDIS_WANDB_COMPACT", "1"):
                ...
                if use_wandb:
                    ...
                    if not _env_flag("EXPDIS_WANDB_COMPACT", "1"):
                        ...
                        ckpt_payload[f"{grpo_metric_prefix}/checkpoint_gcs_base"] = (
                            os.environ.get("EXPDIS_GCS_CHECKPOINT_BASE") or ""
                        ).strip()
            ...
            if _step_due(global_step, reload_every_steps, reload_first_step):
                _maybe_export_and_reload_vllm(state, cfg, m_cfg, global_step, reason="behavior_policy_sync")
                if is_source_host:
                    assert tokenizer is not None
        ...
        force_final_checkpoint = _env_flag("EXPDIS_FORCE_FINAL_PHASE_CHECKPOINT", "0")
        # NOTE: this condition must be rank-independent — _save_checkpoint saves
        # globally-sharded params via Orbax, which requires EVERY process to
        # participate (matching the unguarded in-loop save above). Gating it by
        # is_source_host would deadlock multihost runs at the Orbax barrier.
        if (
            ...
            old_hf_async = os.environ.get("EXPDIS_HF_MIRROR_ASYNC")
            if force_final_checkpoint and _env_flag("EXPDIS_HF_MIRROR_CHECKPOINTS", "0"):
                ...
                os.environ["EXPDIS_HF_MIRROR_ASYNC"] = "0"
            try:
                _save_checkpoint(state, cfg, global_step, rnd_state=rnd_map, upload_to_gcs=True)
            finally:
                if old_hf_async is None:
                    os.environ.pop("EXPDIS_HF_MIRROR_ASYNC", None)
                else:
                    os.environ["EXPDIS_HF_MIRROR_ASYNC"] = old_hf_async
...
def _cleanup_cache_paths(*, aggressive: bool) -> None:
    ...
    if aggressive or os.environ.get("EXPDIS_CKPT_CLEAR_WANDB_CACHE", "1") not in ("0", "false", "False", "no"):
        cache_paths.extend(
            [
                os.path.expanduser("~/wandb"),
                os.path.expanduser("~/expdis/wandb"),
                os.path.expanduser("~/tmx-jax/wandb"),  # legacy pre-rename location
            ]
        )
    ...
    if aggressive and os.environ.get("EXPDIS_CKPT_CLEAR_HF_CACHE_ON_LOW_SPACE", "0") not in (
        "0",
        "false",
        "False",
        "no",
        ...
        for path in (
            ...
            if os.path.exists(path):
                ...
                _remove_tree(path)
    if aggressive and os.environ.get("EXPDIS_CKPT_CLEAR_JAX_CACHE_ON_LOW_SPACE", "1") not in (
        "0",
        "false",
        "False",
        "no",
...
def _cleanup_old_run_dirs(cfg: TrainConfig, min_free_bytes: int) -> None:
    if os.environ.get("EXPDIS_CKPT_DELETE_OLD_RUNS", "0") in ("0", "false", "False", "no"):
        return
...
def _checkpoint_space_janitor(cfg: TrainConfig, ckpt_dir: str, *, aggressive: bool = False) -> None:
    min_gb = float(os.environ.get("EXPDIS_CKPT_MIN_FREE_GB", "24") or "24")
    min_free = int(min_gb * (1024 ** 3))
    keep_last = int(os.environ.get("EXPDIS_LOCAL_CHECKPOINT_KEEP_LAST", "1") or "1")
    keep_reload = int(os.environ.get("EXPDIS_LOCAL_VLLM_RELOAD_KEEP_LAST", "0") or "0")
    run_root = _run_root_for_output(cfg.output_dir)
    roots = {
        os.path.abspath(cfg.output_dir),
        os.path.dirname(os.path.abspath(cfg.output_dir)),
        run_root,
        os.path.expanduser("~/expdis/runs"),
        os.path.expanduser("~/tmx-jax/runs"),  # legacy pre-rename location
    }
...
def _ensure_space_for_vllm_export(cfg: TrainConfig, local_dir: str) -> None:
    ...
    min_gb = float(os.environ.get("EXPDIS_VLLM_EXPORT_MIN_FREE_GB", "28") or "28")
...
def _checkpoint_host_leaf(value: Any) -> Any:
    if isinstance(value, jax.Array):
        try:
            ...
            raise RuntimeError(
                "Cannot serialize a globally sharded array from one local shard; "
                "gather the full array collectively before host checkpoint conversion"
            ) from exc
...
def _save_checkpoint(state, cfg: TrainConfig, step: int, rnd_state=None, *, upload_to_gcs: bool = True):
    """Save an Orbax checkpoint (every host participates) and mirror it.

    A failed local write is retried after freeing disk space, preserving the
    entire payload. Failure of both attempts aborts rather than losing a handoff.
    """
    if cfg.checkpoint_root:
        from .checkpointing import broadcast_source_object, save_shared_checkpoint
        payload = {"params": state.params, "step": int(step)}
        if _env_flag("EXPDIS_SAVE_OPT_STATE", "0"):
            payload["opt_state"] = state.opt_state
        rnd_host = _checkpoint_safe_host_tree(rnd_state) if jax.process_index() == 0 and rnd_state is not None else None
        rnd_host = broadcast_source_object(rnd_host)
        if rnd_host is not None:
            payload["rnd_state"] = rnd_host
        saved = save_shared_checkpoint(cfg, f"step_{step:06d}", payload)
        if jax.process_index() == 0:
            print(f"[ckpt] saved shared stage checkpoint {saved}", flush=True)
        return
    ckpt_dir = os.path.abspath(os.path.join(cfg.output_dir, f"step_{step:06d}"))
    payload = {"params": state.params, "step": int(step)}
    save_opt_state = os.environ.get("EXPDIS_SAVE_OPT_STATE", "0") not in ("0", "false", "False")
    keep_last = int(os.environ.get("EXPDIS_LOCAL_CHECKPOINT_KEEP_LAST", "1") or "1")
    if save_opt_state:
        payload["opt_state"] = state.opt_state
    ...
    direct_gcs = (
        upload_to_gcs
        and _env_flag("EXPDIS_ALLOW_GCS_ARTIFACTS", "0")
        and jax.process_count() > 1
        and _env_flag("EXPDIS_ORBAX_DIRECT_GCS_CHECKPOINT", "0")
    )
    if jax.process_count() > 1 and not upload_to_gcs and _env_flag("EXPDIS_SKIP_MULTIHOST_LOCAL_ONLY_CHECKPOINT", "1"):
        print(
    ...
    if direct_gcs:
        ...
        try:
            ...
            if os.environ.get("EXPDIS_REQUIRE_GCS_CHECKPOINT_UPLOAD", "1") not in ("0", "false", "False", "no"):
                raise
            _maybe_upload_trajectory_snapshot(cfg, step)
            _maybe_upload_training_metrics_snapshot(cfg, step)
            return
    try:
        ...
        if upload_to_gcs and not _env_flag("EXPDIS_ALLOW_GCS_ARTIFACTS", "0"):
            if _env_flag("EXPDIS_HF_MIRROR_CHECKPOINTS", "0"):
                _maybe_upload_path_to_hf(
                    ckpt_dir,
                    cfg,
        ...
        if upload_to_gcs:
            ...
            if not ok and os.environ.get("EXPDIS_REQUIRE_GCS_CHECKPOINT_UPLOAD", "1") not in ("0", "false", "False", "no"):
        ...
        try:
            print("[ckpt] retrying complete checkpoint after cleanup", flush=True)
            _save_checkpoint_payload(ckpt_dir, retry_payload)
            print(f"[ckpt] saved {ckpt_dir} after cleanup", flush=True)
            if upload_to_gcs and _env_flag("EXPDIS_ALLOW_GCS_ARTIFACTS", "0"):
                ok = _maybe_upload_checkpoint_to_gcs(ckpt_dir, cfg)
                if not ok and os.environ.get("EXPDIS_REQUIRE_GCS_CHECKPOINT_UPLOAD", "1") not in ("0", "false", "False", "no"):
            ...
            raise RuntimeError(f"checkpoint save failed after retry: {ckpt_dir}") from retry_e


def _maybe_upload_checkpoint_to_gcs(ckpt_dir: str, cfg: TrainConfig) -> bool:
    gcs_target = _gcs_checkpoint_target(ckpt_dir, cfg)
    max_attempts = int(os.environ.get("EXPDIS_GCS_UPLOAD_MAX_ATTEMPTS", "5") or "5")
    retry_seconds = float(os.environ.get("EXPDIS_GCS_UPLOAD_RETRY_SECONDS", "30") or "30")
    attempt = 0
...
def _hf_run_prefix(cfg: TrainConfig) -> str:
    ...
    return (os.environ.get("EXPDIS_HF_ARTIFACT_PREFIX") or default).strip().strip("/")
...
def _maybe_upload_training_metrics_snapshot(cfg: TrainConfig, step: int) -> None:
    ...
    if not os.path.exists(metrics):
        return
    if not _env_flag("EXPDIS_ALLOW_GCS_ARTIFACTS", "0"):
        if _env_flag("EXPDIS_HF_MIRROR_METRICS", "0"):
            for target_name in (
