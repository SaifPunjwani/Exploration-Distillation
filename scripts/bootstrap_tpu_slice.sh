...
if [ "$SKIP_TPU_VALIDATE" != "1" ]; then
  if ! run_remote_command "cd '$REMOTE_ROOT' && . '$VENV_DIR/bin/activate' && export PJRT_DEVICE=TPU && export EXPDIS_TPU_SLICE_LAUNCHED=1 && export EXPDIS_REQUIRE_TPU=1 && export EXPDIS_ALLOW_SINGLE_WORKER_TPU=1 && export TPU_SKIP_MDS_QUERY=1 && export TPU_VISIBLE_CHIPS=0 && export TPU_PROCESS_BOUNDS=1,1,1 && export TPU_CHIPS_PER_PROCESS_BOUNDS=1,1,1 && export TPU_CHIPS_PER_HOST_BOUNDS=1,1,1 && export TPU_HOST_BOUNDS=1,1,1 && export TPU_WORKER_HOSTNAMES=localhost && timeout 90s python -c 'from expdis_torch.devices import resolve_device; dev, is_xla, label = resolve_device(\"tpu\"); print(\"resolved\", dev, is_xla, label)'"
  then
