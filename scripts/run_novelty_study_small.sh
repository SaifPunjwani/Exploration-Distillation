...
if [ -n "${DATASET_NAME:-}" ] && [ -z "${DATASETS:-}" ]; then
  DATASETS="${DATASET_NAME}"
fi
DATASETS="${DATASETS:-deepscaler_preview dapo_math_17k}"
RUN_TAG="${RUN_TAG:-novelty_small}"
...
EMBEDDING_MAX_TEXTS="${EMBEDDING_MAX_TEXTS:-384}"
EMBEDDING_MAX_LENGTH="${EMBEDDING_MAX_LENGTH:-256}"
EMBEDDING_CLUSTER_K="${EMBEDDING_CLUSTER_K:-8}"
EMBEDDING_PERM_ITERS="${EMBEDDING_PERM_ITERS:-400}"
...
RUN_NAME_PREFIX="${RUN_NAME_PREFIX:-EXP-SMALL-DEC-SFTGRPO-BASEGRPO}"
...
echo "Small novelty study complete. Run aggregation with:"
