...
# Target: a v5litepod-64 slice (64 chips, 16 hosts, 4 chips/host); set POD_TPU_NAME/POD_SSH_HOST
#
# Architecture:
#   - Workers 1-15: vLLM inference (TP=4 each, 60 chips)
#   - Worker 0: TPU training with flash attention (1 chip, 16GB HBM)
#   - ~6% less inference throughput vs 16 servers, but 10-50x faster training
set -euo pipefail
...
POD_ZONE="${POD_ZONE:-europe-west4-b}"
...
RUN_NAME="${RUN_NAME:-v5lite_single_pod_$(date +%Y%m%d_%H%M%S)}"
WANDB_PROJECT="${WANDB_PROJECT:-two-model-explore}"
WANDB_ENTITY="${WANDB_ENTITY:-}"
WANDB_RUN_NAME="${WANDB_RUN_NAME:-$RUN_NAME}"
RUN_ARTIFACT_GCS_PREFIX="${RUN_ARTIFACT_GCS_PREFIX:-gs://two-model-exploration-checkpoints/v5lite_single_pod}"
...
if [ "$BOOTSTRAP_TRAINING" = "1" ]; then
  ...
  ssh -o StrictHostKeyChecking=no "$POD_SSH_HOST" \
    "cd '$REMOTE_ROOT' && VENV_DIR=.venv_tpu_slice USE_TPU=1 bash scripts/bootstrap_tpu_env.sh"
fi
...
# === Flash attention for 8k training on 16GB chip ===
...
# Grad ckpt OFF for speed — if OOM on 16GB, set both to 1
