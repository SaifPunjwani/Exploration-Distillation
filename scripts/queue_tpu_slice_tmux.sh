...
kill_existing_sessions() {
  ...
  for worker in "${WORKERS[@]}"; do
    tmx_gcloud alpha compute tpus tpu-vm ssh \
      ...
      --zone="$ZONE" \
      "$TPU_TRANSPORT_FLAG" \
      --command="pkill -f 'python -u main.py --device [t]pu' || true; pkill -f 'python main.py --device [t]pu' || true; pkill -f 'python -c import torch_xla.core.xla_model as x[m]' || true; tmux ls 2>/dev/null | sed 's/:.*//' | grep '^${SESSION_PREFIX}_' | xargs -r -n1 tmux kill-session -t || true"
  done
}
