...
WORKER="${WORKER:-0}"
TPU_ACCELERATOR_TYPE="${TPU_ACCELERATOR_TYPE:-$(expdis_resolve_tpu_accelerator_type 2>/dev/null || true)}"
...
launch_eval_run() {
  local run_name="$1"
  local model_name="$2"
  local remote_script="$REMOTE_ROOT/.expdis_launch_${run_name}.sh"
