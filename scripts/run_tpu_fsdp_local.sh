...
infer_accelerator_type() {
  python3 - <<'PY'
from expdis_torch.devices import _infer_tpu_accelerator_type
...
if [ -z "${EXPDIS_XLA_PERSISTENT_CACHE_DIR:-}" ]; then
  ...
  export EXPDIS_XLA_PERSISTENT_CACHE_DIR="/tmp/expdis_xla_cache_fsdp_${sanitized_chips}"
fi

run_cfg="$(mktemp /tmp/expdis_accelerate_fsdp_XXXX.yaml)"
...
echo "[tpu-fsdp] PJRT_DEVICE=$PJRT_DEVICE TPU_VISIBLE_CHIPS=${TPU_VISIBLE_CHIPS:-<rank-mapped>} cache=${EXPDIS_XLA_PERSISTENT_CACHE_DIR:-}"
