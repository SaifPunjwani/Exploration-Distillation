...
if [ -z "$TMX_VLLM_CONCURRENCY_VALUE" ]; then
  ...
  if [ "$TMX_VLLM_CONCURRENCY_VALUE" -lt 4 ]; then
    TMX_VLLM_CONCURRENCY_VALUE=4
  fi
fi
log "Derived TMX_VLLM_CONCURRENCY=$TMX_VLLM_CONCURRENCY_VALUE from healthy_server_count=$healthy_server_count"
...
COMMON_EXPORTS="export WANDB_PROJECT=two-model-explore && \\
export WANDB_ENTITY='${WANDB_ENTITY:-}' && \\
export TOKENIZERS_PARALLELISM=false && \\
export PYTHONUNBUFFERED=1 && \\
export TMX_GRPO_GRAD_CKPT='${TMX_GRPO_GRAD_CKPT:-0}' && \\
export TMX_GRPO_LOGGING_STRATEGY=steps && \\
export TMX_GRPO_LOGGING_STEPS=1 && \\
export TMX_GRPO_SAVE_STRATEGY='${TMX_GRPO_SAVE_STRATEGY:-no}' && \\
export TMX_GRPO_SAVE_STEPS='${TMX_GRPO_SAVE_STEPS:-16}' && \\
export TMX_SKIP_INTERMEDIATE_EVALS=0 && \\
export TMX_DISABLE_BF16='${TMX_DISABLE_BF16:-0}' && \\
export TMX_ENABLE_THINKING='${TMX_ENABLE_THINKING:-false}' && \\
export TMX_OPTIM='${TMX_OPTIM:-adamw_torch}' && \\
export TMX_DEBUG_GENERATE='${TMX_DEBUG_GENERATE:-0}' && \\
export TMX_ABORT_ON_DEGENERATE_EXPLORER='${TMX_ABORT_ON_DEGENERATE_EXPLORER:-1}' && \\
export TMX_ABORT_ON_BAD_TRAJ_WINDOW='${TMX_ABORT_ON_BAD_TRAJ_WINDOW:-1}' && \\
export TMX_FEW_SHOT_EXAMPLES='${TMX_FEW_SHOT_EXAMPLES:-0}' && \\
...
export TMX_GCS_CHECKPOINT_BASE='${TMX_GCS_CHECKPOINT_BASE:-gs://llmvae-checkpoints/two-model-exploration/checkpoints}'"

# main.py arguments (shared between CPU and TPU modes)
MAIN_ARGS="--use-wandb \\
  --seed 0 \\
...
export TMX_XLA_PERSISTENT_CACHE_DIR=/tmp/tmx_xla_cache_fsdp_spmd && \\
mkdir -p '$RUN_DIR' && \\
python -u main.py \\
  --device tpu \\
  $MAIN_ARGS \\
