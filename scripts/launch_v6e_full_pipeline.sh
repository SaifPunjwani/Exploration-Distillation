...
source "$HOME/.venv_tpu_slice/bin/activate"
source .env 2>/dev/null || true

# === Full Pipeline: Explorer GRPO (500 steps) → Trajectories → SFT → Online GRPO (250) → Eval ===
# Saturation study: loss 2.04→0.99 over 1000 steps, correctness flat ~45%.
# 500 steps captures 66% of loss improvement; further gains marginal.
# Full run from scratch — no checkpoint resume.
...
# === Fresh run (no Explorer resume — train from scratch) ===
export TMX_RESUME_STEP=0
export TMX_RESUME_CHECKPOINT=""
export TMX_RESUME_RUN_NAME=""
export TMX_RESUME_WANDB_ID=""

# === GCS checkpoint backup ===
export TMX_GCS_CHECKPOINT_ROOT=gs://two-model-exploration-checkpoints/v6e_full_pipeline

# === Trajectories: collect fresh from Explorer after GRPO training ===
export TMX_PRECOMPUTED_TRAJ_PATH=""

# === FULL PIPELINE: do NOT stop after Explorer ===
...
# === Trajectory early stopping: collect ~2048 accepted before moving on ===
...
# === SFT: eager attention (Pallas backward OOMs single chip) + Adafactor ===
# distill.py auto-swaps flash→eager for training, restores after.
# Adafactor saves ~5G vs AdamW optimizer states.
export TMX_DISTILL_OPTIM=adafactor
export TMX_DISTILL_GRAD_CKPT=1
export TMX_DISTILL_CE_CHUNK=256
...
RUN_NAME="v6e_full_pipeline_$(date +%Y%m%d_%H%M%S)"
RUN_DIR="runs/${RUN_NAME}"
mkdir -p "${RUN_DIR}"

echo "=== Full Pipeline: Explorer GRPO (500) → Trajectories → SFT → Online GRPO (250) → Eval ===" | tee -a "${RUN_DIR}/console.log"
echo "Run: ${RUN_NAME}" | tee -a "${RUN_DIR}/console.log"
echo "Explorer: fresh from base (500 GRPO steps)" | tee -a "${RUN_DIR}/console.log"
