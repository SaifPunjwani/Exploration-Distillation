...
infer_worker_count() {
  local describe_json
  describe_json="$(expdis_gcloud alpha compute tpus tpu-vm describe "$INFER_TPU_NAME" --zone "$INFER_ZONE" --format=json)"
...
check_host() {
  ...
  run_ssh "$host" "
    ...
    echo '---';
    echo '[proc]';
    ps aux | grep -E 'run_expdis_torch.py|vllm|ray|accelerate' | grep -v grep | head -40 || true;
    echo '---';
    echo '[disk]';
    df -h / | tail -1;
  "
...
clean_train_host() {
  ...
  remote_process_cleanup "$host" $'python -u run_expdis_torch.py --device tpu\npython run_expdis_torch.py --device tpu\naccelerate launch\nvllm serve\nsite-packages/ray/\nray::'
  run_ssh "$host" "
    fuser -k /dev/vfio/0 /dev/vfio/1 /dev/vfio/2 /dev/vfio/3 2>/dev/null || true
    rm -rf /tmp/expdis_xla_cache_* /tmp/hf_cache 2>/dev/null || true
    echo '[post-clean]'
    tmux ls 2>/dev/null || true
    echo '---'
    ps aux | grep -E 'run_expdis_torch.py|vllm|ray|accelerate' | grep -v grep | head -20 || true
    echo '---'
    df -h / | tail -1
  "
}
