...
python3 -m py_compile \
  expdis_torch/config.py \
  expdis_torch/wandb_metrics.py \
  expdis_torch/explorer.py \
  expdis_torch/distill.py \
  expdis_torch/pipeline.py \
...
rg -n 'EXPDIS_STOP_AFTER_EXPLORER=1' scripts/launch_qwen3_explorer_novelty_saturation.sh >/dev/null
rg -n 'EXPDIS_STOP_AFTER_TRAJECTORIES=0' scripts/launch_qwen3_explorer_novelty_saturation.sh >/dev/null
rg -n 'EXPDIS_STOP_AFTER_EXPLORER=1' scripts/launch_qwen3_explorer_fit_probe.sh >/dev/null
rg -n 'EXPDIS_TRAJ_USE_VLLM="\$\{EXPDIS_TRAJ_USE_VLLM:-1\}"' scripts/launch_qwen3_explorer_novelty_saturation.sh >/dev/null
rg -n 'EXPDIS_GRPO_USE_VLLM="\$\{EXPDIS_GRPO_USE_VLLM:-1\}"' scripts/launch_qwen3_explorer_novelty_saturation.sh >/dev/null
rg -n 'EXPDIS_GRPO_USE_VLLM="\$\{EXPDIS_GRPO_USE_VLLM:-1\}"' scripts/launch_qwen3_one_round_full_dapo.sh >/dev/null
rg -n 'EXPDIS_OPTIM="\$\{EXPDIS_OPTIM:-adafactor\}"' scripts/launch_qwen3_explorer_novelty_saturation.sh >/dev/null
rg -n 'EXPDIS_OPTIM="\$\{EXPDIS_OPTIM:-adafactor\}"' scripts/launch_qwen3_one_round_full_dapo.sh >/dev/null
rg -n 'EXPDIS_EXPLORER_VLLM_TOP_P="\$\{EXPDIS_EXPLORER_VLLM_TOP_P:-0.95\}"' scripts/launch_qwen3_explorer_novelty_saturation.sh >/dev/null
rg -n 'EXPDIS_EXPLORER_VLLM_TOP_K="\$\{EXPDIS_EXPLORER_VLLM_TOP_K:-20\}"' scripts/launch_qwen3_explorer_novelty_saturation.sh >/dev/null
rg -n 'EXPDIS_TRAJ_VLLM_TOP_P="\$\{EXPDIS_TRAJ_VLLM_TOP_P:-0.95\}"' scripts/launch_qwen3_explorer_novelty_saturation.sh >/dev/null
rg -n 'EXPDIS_TRAJ_VLLM_TOP_K="\$\{EXPDIS_TRAJ_VLLM_TOP_K:-20\}"' scripts/launch_qwen3_explorer_novelty_saturation.sh >/dev/null
rg -n 'EXPDIS_EXPLORER_TRAIN_TOP_P="\$\{EXPDIS_EXPLORER_TRAIN_TOP_P:-0.95\}"' scripts/launch_qwen3_explorer_novelty_saturation.sh >/dev/null
rg -n 'EXPDIS_EXPLORER_TRAIN_TOP_K="\$\{EXPDIS_EXPLORER_TRAIN_TOP_K:-20\}"' scripts/launch_qwen3_explorer_novelty_saturation.sh >/dev/null
rg -n 'EXPDIS_EXPLORER_VLLM_TOP_P="\$\{EXPDIS_EXPLORER_VLLM_TOP_P:-0.95\}"' scripts/launch_qwen3_one_round_full_dapo.sh >/dev/null
rg -n 'EXPDIS_EXPLORER_VLLM_TOP_K="\$\{EXPDIS_EXPLORER_VLLM_TOP_K:-20\}"' scripts/launch_qwen3_one_round_full_dapo.sh >/dev/null
rg -n 'EXPDIS_TRAJ_VLLM_TOP_P="\$\{EXPDIS_TRAJ_VLLM_TOP_P:-0.95\}"' scripts/launch_qwen3_one_round_full_dapo.sh >/dev/null
rg -n 'EXPDIS_TRAJ_VLLM_TOP_K="\$\{EXPDIS_TRAJ_VLLM_TOP_K:-20\}"' scripts/launch_qwen3_one_round_full_dapo.sh >/dev/null
rg -n 'EXPDIS_EXPLORER_TRAIN_TOP_P="\$\{EXPDIS_EXPLORER_TRAIN_TOP_P:-0.95\}"' scripts/launch_qwen3_one_round_full_dapo.sh >/dev/null
rg -n 'EXPDIS_EXPLORER_TRAIN_TOP_K="\$\{EXPDIS_EXPLORER_TRAIN_TOP_K:-20\}"' scripts/launch_qwen3_one_round_full_dapo.sh >/dev/null
rg -n 'EXPDIS_EXPLORER_DECOUPLED_GRPO=1' scripts/launch_qwen3_explorer_novelty_saturation.sh >/dev/null
rg -n 'EXPDIS_EXPLORER_DECOUPLED_GRPO=1' scripts/launch_qwen3_one_round_full_dapo.sh >/dev/null
rg -n 'EXPDIS_EXPLORER_DECOUPLED_TRAIN_FSDP=0' scripts/launch_qwen3_explorer_novelty_saturation.sh >/dev/null
rg -n 'EXPDIS_EXPLORER_DECOUPLED_TRAIN_FSDP=0' scripts/launch_qwen3_one_round_full_dapo.sh >/dev/null
rg -n 'EXPDIS_DECOUPLED_TRAIN_TOKEN_CHUNK="\$\{EXPDIS_DECOUPLED_TRAIN_TOKEN_CHUNK:-256\}"' scripts/launch_qwen3_explorer_novelty_saturation.sh >/dev/null
rg -n 'EXPDIS_DECOUPLED_TRAIN_TOKEN_CHUNK="\$\{EXPDIS_DECOUPLED_TRAIN_TOKEN_CHUNK:-256\}"' scripts/launch_qwen3_one_round_full_dapo.sh >/dev/null
rg -n 'EXPDIS_DECOUPLED_GRAD_CKPT="\$\{EXPDIS_DECOUPLED_GRAD_CKPT:-0\}"' scripts/launch_qwen3_explorer_novelty_saturation.sh >/dev/null
rg -n 'EXPDIS_DECOUPLED_GRAD_CKPT="\$\{EXPDIS_DECOUPLED_GRAD_CKPT:-0\}"' scripts/launch_qwen3_one_round_full_dapo.sh >/dev/null
rg -n 'EXPDIS_DECOUPLED_ACCUM_MARK_STEP="\$\{EXPDIS_DECOUPLED_ACCUM_MARK_STEP:-0\}"' scripts/launch_qwen3_explorer_novelty_saturation.sh >/dev/null
rg -n 'EXPDIS_DECOUPLED_ACCUM_MARK_STEP="\$\{EXPDIS_DECOUPLED_ACCUM_MARK_STEP:-0\}"' scripts/launch_qwen3_one_round_full_dapo.sh >/dev/null
rg -n 'EXPDIS_DECOUPLED_GRAD_CKPT=1' scripts/launch_qwen3_explorer_novelty_saturation.sh >/dev/null
rg -n 'EXPDIS_DECOUPLED_GRAD_CKPT=1' scripts/launch_qwen3_one_round_full_dapo.sh >/dev/null
...
rg -n "EXPDIS_TRAJ_USE_VLLM='\\\$\\{EXPDIS_TRAJ_USE_VLLM:-0\\}'" scripts/launch_dual_slice_training.sh >/dev/null
rg -n 'SKIP_INFERENCE_SLICE="\$\{SKIP_INFERENCE_SLICE:-0\}"' scripts/launch_dual_slice_training.sh >/dev/null
rg -n "EXPDIS_GRPO_LOGPROB_CHUNK_SIZE='\\\$\\{EXPDIS_GRPO_LOGPROB_CHUNK_SIZE:-2\\}'" scripts/launch_dual_slice_training.sh >/dev/null
rg -n "EXPDIS_DECOUPLED_ACCUM_MARK_STEP='\\\$\\{EXPDIS_DECOUPLED_ACCUM_MARK_STEP:-1\\}'" scripts/launch_dual_slice_training.sh >/dev/null
rg -n "EXPDIS_DECOUPLED_ACCUM_MARK_RESET_SCOPE='\\\$\\{EXPDIS_DECOUPLED_ACCUM_MARK_RESET_SCOPE:-0\\}'" scripts/launch_dual_slice_training.sh >/dev/null
...
rg -n -F '\boxed{}' expdis_torch/prompting.py >/dev/null
rg -n 'enable_thinking' expdis_torch/prompting.py >/dev/null
...
rg -n 'EXPDIS_SKIP_INTERMEDIATE_EVALS=1' scripts/launch_qwen3_explorer_novelty_saturation.sh >/dev/null
rg -n 'EXPDIS_SKIP_INTERMEDIATE_EVALS=1' scripts/launch_qwen3_one_round_full_dapo.sh >/dev/null
rg -n 'EXPDIS_SKIP_FINAL_BENCHMARK_EVALS=1' scripts/launch_qwen3_explorer_novelty_saturation.sh >/dev/null
rg -n 'EXPDIS_SKIP_FINAL_BENCHMARK_EVALS=1' scripts/launch_qwen3_one_round_full_dapo.sh >/dev/null
rg -n 'EXPDIS_FINAL_EVAL_ONLY_ACTUAL=1' scripts/launch_qwen3_one_round_full_dapo.sh >/dev/null
rg -n 'EXPDIS_FINAL_EVAL_MAX_EXAMPLES=1' scripts/launch_qwen3_one_round_full_dapo.sh >/dev/null
rg -n 'EXPDIS_WANDB_MINIMAL=1' scripts/launch_qwen3_explorer_novelty_saturation.sh >/dev/null
rg -n 'EXPDIS_WANDB_MINIMAL=1' scripts/launch_qwen3_one_round_full_dapo.sh >/dev/null
...
rg -n 'expdis_gcloud alpha compute tpus tpu-vm ssh "\$INFER_TPU_NAME"' scripts/qwen3_serious_remote_state.sh >/dev/null
...
rg -n 'actual_online_grpo_max_steps' expdis_torch/config.py expdis_torch/distill.py >/dev/null
rg -n 'actual_train_mode: str = "grpo"' expdis_torch/config.py >/dev/null
rg -n 'sft_online_grpo' expdis_torch/config.py expdis_torch/pipeline.py >/dev/null
rg -n 'stopped_after_explorer' expdis_torch/pipeline.py >/dev/null
rg -n 'online_grpo_uses_dataset_prompts' expdis_torch/pipeline.py >/dev/null
rg -n 'grpo_metric_prefix: str = "grpo"' expdis_torch/explorer.py >/dev/null
rg -n 'train_metric_prefix: Optional\[str\] = "train"' expdis_torch/explorer.py >/dev/null
rg -n 'actual_grpo/reward/' expdis_torch/wandb_metrics.py >/dev/null

echo "[validate-qwen3] checkpoint policy"
rg -n 'EXPDIS_ACTUAL_GRPO_SAVE_STEPS="\$\{EXPDIS_ACTUAL_GRPO_SAVE_STEPS:-25\}"' scripts/launch_qwen3_one_round_full_dapo.sh >/dev/null
rg -n 'EXPDIS_ACTUAL_GRPO_SAVE_TOTAL_LIMIT="\$\{EXPDIS_ACTUAL_GRPO_SAVE_TOTAL_LIMIT:-4\}"' scripts/launch_qwen3_one_round_full_dapo.sh >/dev/null
rg -n '_save_model_to_gcs_and_reload_vllm\(explorer_model, tokenizer, _run_name, role="explorer"\)' expdis_torch/pipeline.py >/dev/null
rg -n '_save_model_to_gcs_and_reload_vllm\(actual_model, tokenizer, _run_name, role="actual"\)' expdis_torch/pipeline.py >/dev/null
rg -n 'EXPDIS_ACTUAL_GRPO_SAVE_STEPS' expdis_torch/distill.py >/dev/null
