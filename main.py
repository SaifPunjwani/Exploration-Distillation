#!/usr/bin/env python
"""
Entry point for two-model exploration (Explorer + Actual) with TPU support.
...
from tmx.config import build_configs_from_args, parse_args
...
def _maybe_initialize_xla_persistent_cache() -> None:
    cache_dir = (os.environ.get("TMX_XLA_PERSISTENT_CACHE_DIR") or "").strip()
    if cache_dir.lower() in {"0", "false", "no", "off", "disable", "__disable__"}:
        return
    if not cache_dir:
        return
    if os.environ.get("TMX_XLA_PERSISTENT_CACHE_INITIALIZED") == "1":
        return
    if os.environ.get("PJRT_DEVICE", "").upper() != "TPU":
        return
    readonly = str(os.environ.get("TMX_XLA_PERSISTENT_CACHE_READONLY", "0")).strip().lower() in (
        "1",
        "true",
        "yes",
    )
    try:
        ...
        os.environ["TMX_XLA_PERSISTENT_CACHE_INITIALIZED"] = "1"
...
def _maybe_configure_tpu_host_threads() -> None:
    if os.environ.get("TMX_TPU_HOST_THREADS_CONFIGURED") == "1":
        return
    wants_tpu = os.environ.get("PJRT_DEVICE", "").upper() == "TPU" or _argv_requests_tpu(sys.argv[1:])
    if not wants_tpu:
        return

    host_threads = _parse_positive_int_env("TMX_TPU_HOST_TORCH_THREADS", 4)
    interop_threads = _parse_positive_int_env("TMX_TPU_HOST_TORCH_INTEROP_THREADS", 1)
    ...
    os.environ["TMX_TPU_HOST_THREADS_CONFIGURED"] = "1"


def _maybe_enable_xla_spmd() -> None:
    """Enable SPMD mode for XLA FSDP v2 if TMX_FSDP is set on TPU.

    Skipped when the decoupled Explorer path is active (TMX_EXPLORER_DECOUPLED_GRPO),
    ...
    """
    fsdp_mode = (os.environ.get("TMX_FSDP") or "").strip().lower()
    ...
    decoupled = (os.environ.get("TMX_EXPLORER_DECOUPLED_GRPO") or "").strip().lower()
...
def main():
    ...
    from tmx.pipeline import run_pipeline
