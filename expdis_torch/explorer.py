...
def _get_or_create_model_shell(model_name: str, is_xla: bool):
    ...
    if is_xla:
        use_flash = os.environ.get("EXPDIS_XLA_FLASH_ATTENTION", "0") not in ("0", "false", "no", "")
        cfg_copy.attn_implementation = "tpu_flash" if use_flash else "eager"
    shell = cls(cfg_copy)
    _use_flash = is_xla and os.environ.get("EXPDIS_XLA_FLASH_ATTENTION", "0") not in ("0", "false", "no", "")
    if _use_flash:
        from expdis_torch.utils import _patch_model_bf16_for_flash_attention
...
def _should_build_novelty_scorer(cfg) -> bool:
    if _env_flag("EXPDIS_FORCE_NOVELTY_SCORER", default=False):
        return True
    try:
...
def _use_cpu_novelty(is_xla: bool) -> bool:
    ...
    return not _env_flag("EXPDIS_NOVELTY_ON_TPU", default=False) if is_xla else False
...
def _pad_decoupled_batch_to_global_widths(
    ...
    *,
    ...
    bucket = int(os.environ.get("EXPDIS_XLA_PAD_BUCKET", "256") or 256)
...
def _wrap_model_with_xla_fsdp(explorer_model, *, use_grad_ckpt: bool):
    ...
    wrap_cls_override = (os.environ.get("EXPDIS_FSDP_WRAP_CLS") or "").strip()
    ...
    use_bf16 = os.environ.get("EXPDIS_DISABLE_BF16", "0") != "1"
...
def _save_xla_fsdp_full_checkpoint(model, tokenizer, output_dir: str) -> None:
    ...
    debug_progress = _env_flag("EXPDIS_DECOUPLED_TRAIN_DEBUG_PROGRESS", default=False)
    ...
    xm_mod.rendezvous("expdis_decoupled_fsdp_save_start")
    ...
    xm_mod.rendezvous("expdis_decoupled_fsdp_shards_written")
    ...
    xm_mod.rendezvous("expdis_decoupled_fsdp_full_saved")
...
def _get_per_token_logps_chunked(
    ...
    chunk_spec = (os.environ.get("EXPDIS_GRPO_LOGPROB_CHUNK_SIZE") or "").strip()
    if not chunk_spec:
...
def _rebuild_explorer_model_for_generation(
    ...
    if is_xla and os.environ.get("EXPDIS_XLA_FLASH_ATTENTION", "0") not in ("0", "false", "no", ""):
    ...
    if answer_constraints is not None:
        rebuilt_model._expdis_answer_constraints = answer_constraints
    ...
    if os.environ.get("EXPDIS_XLA_SAFE_GENERATE", "1") != "0":
    ...
    _vllm_active = bool((os.environ.get("EXPDIS_VLLM_SERVER_URLS") or "").strip())
    if _vllm_active:
...
def _build_explorer_model_from_state(
    ...
    *,
    ...
    if patch_generate and os.environ.get("EXPDIS_XLA_SAFE_GENERATE", "1") != "0":
        patch_model_generate_for_xla(rebuilt_model)
...
def _compute_completion_logps(
    ...
    *,
    ...
    progress_enabled = (os.environ.get("EXPDIS_LOGPROB_PROGRESS") or "1").strip().lower() not in ("0", "false", "no")
    last_chunk_idx = 0
...
def _collate_explorer_rollout_groups(batch, pad_token_id: int):
    ...
    _bucket = int(os.environ.get("EXPDIS_XLA_PAD_BUCKET", "256") or 256)
...
def _save_explorer_step_checkpoint(model, tokenizer, output_dir: str, step: int, novelty_scorer=None) -> None:
    ...
    # Upload to GCS if configured, then enforce local limit.
    gcs_root = os.environ.get("EXPDIS_GCS_CHECKPOINT_ROOT", "")
    if gcs_root:
        import subprocess
        ...
        try:
            ...
            print(f"[checkpoint] uploaded to {gcs_dst}", flush=True)
        except Exception as e:
            print(f"[checkpoint] GCS upload failed: {e}", flush=True)
    ...
    save_limit = int(os.environ.get("EXPDIS_GRPO_SAVE_TOTAL_LIMIT", "3") or 3)
    if save_limit > 0:
...
def _generate_explorer_rollout_groups(
    ...
    *,
    ...
    from expdis_torch.vllm_generate import generate_completions_vllm
    ...
    vllm_urls = [u.strip() for u in ((os.environ.get("EXPDIS_VLLM_SERVER_URLS") or "").split(",")) if u.strip()]
    use_ondevice_gen = not vllm_urls
    if use_ondevice_gen:
        print("[decoupled-explorer] No vLLM URLs set, using on-device generation.", flush=True)
        ondevice_gen_batch = max(1, int(os.environ.get("EXPDIS_DECOUPLED_ONDEVICE_GEN_BATCH", "2") or 2))
    vllm_model = os.environ.get("EXPDIS_VLLM_MODEL_NAME", "")
    vllm_api_key = os.environ.get("EXPDIS_VLLM_API_KEY", "")
    vllm_concurrency = int(os.environ.get("EXPDIS_VLLM_CONCURRENCY", "64"))
    vllm_top_p = float(os.environ.get("EXPDIS_EXPLORER_VLLM_TOP_P", "0.95") or 0.95)
    vllm_top_k = int(os.environ.get("EXPDIS_EXPLORER_VLLM_TOP_K", "20") or 20)
    num_generations = max(2, int(cfg.grpo_num_generations))
    rollout_prompt_batch = max(1, int(os.environ.get("EXPDIS_EXPLORER_DECOUPLED_ROLLOUT_PROMPTS", getattr(cfg, "trajectory_batch_size", 4)) or 4))
    ...
    logprob_chunk = max(1, int(os.environ.get("EXPDIS_EXPLORER_DECOUPLED_LOGPROB_BATCH", "2") or 2))
    groups = []
    ...
    for start in range(0, len(train_ds), rollout_prompt_batch):
        ...
        # EXPDIS_SKIP_VLLM_LOGPROBS=1 forces XLA re-scoring so old_logprobs match the current training
        ...
        skip_vllm_lp = _env_flag("EXPDIS_SKIP_VLLM_LOGPROBS", default=False)
        ...
        if (os.environ.get("EXPDIS_GRPO_BETA") or "").strip():
...
def _train_explorer_from_rollout_groups(
    ...
    *,
    ...
    use_xla_fsdp = (
        is_xla
        and _env_flag("EXPDIS_EXPLORER_DECOUPLED_TRAIN_FSDP", default=_env_flag("EXPDIS_FSDP", default=False))
    ...
    batch_groups = max(1, int(os.environ.get("EXPDIS_EXPLORER_DECOUPLED_BATCH_GROUPS", str(getattr(cfg, "grpo_batch_size", 1) or 1)) or 1))
    grad_accum = max(
        1,
        int(
            os.environ.get(
                "EXPDIS_EXPLORER_DECOUPLED_GRAD_ACCUM",
    ...
    row_microbatch = max(
        ...
        int(
            os.environ.get(
                "EXPDIS_EXPLORER_DECOUPLED_ROW_MICROBATCH",
                os.environ.get("EXPDIS_EXPLORER_DECOUPLED_TRAIN_ROWS_PER_FWD", "0"),
            )
            or 0
        ),
    )
    beta_env = (os.environ.get("EXPDIS_GRPO_BETA") or "").strip()
    beta_val = float(beta_env) if beta_env else 0.0
    debug_progress = _env_flag("EXPDIS_DECOUPLED_TRAIN_DEBUG_PROGRESS", default=False)
    row_mark_step = is_xla and _env_flag("EXPDIS_DECOUPLED_ROW_MARK_STEP", default=True)
    accum_mark_step = is_xla and _env_flag("EXPDIS_DECOUPLED_ACCUM_MARK_STEP", default=use_xla_fsdp)
    accum_mark_reset_scope = _env_flag("EXPDIS_DECOUPLED_ACCUM_MARK_RESET_SCOPE", default=False)
    pre_optimizer_mark_step = is_xla and _env_flag("EXPDIS_DECOUPLED_PRE_OPTIMIZER_MARK_STEP", default=use_xla_fsdp)
    token_chunk_size = max(
        0,
        int(
            os.environ.get(
                "EXPDIS_DECOUPLED_TRAIN_TOKEN_CHUNK",
    ...
    pad_id = tokenizer.pad_token_id if tokenizer.pad_token_id is not None else 0
    _use_flash = is_xla and os.environ.get("EXPDIS_XLA_FLASH_ATTENTION", "0") not in ("0", "false", "no", "")
    fixed_shapes = (use_xla_fsdp or _use_flash) and _env_flag("EXPDIS_DECOUPLED_TRAIN_FIXED_SHAPES", default=True)
    fixed_prompt_width = int(
        os.environ.get(
            "EXPDIS_DECOUPLED_TRAIN_FIXED_PROMPT_WIDTH",
    ...
    fixed_completion_width = int(
        os.environ.get(
            "EXPDIS_DECOUPLED_TRAIN_FIXED_COMPLETION_WIDTH",
            str(
                getattr(
                    cfg,
    ...
    precollate_batches = _env_flag("EXPDIS_DECOUPLED_PRECOLLATE_BATCHES", default=use_xla_fsdp)
    ...
    # Controllable via EXPDIS_DECOUPLED_GRAD_CKPT (default=1). Disable for short contexts
    # to avoid the ~15 min XLA compilation overhead of the checkpointed graph.
    _enable_grad_ckpt = (os.environ.get("EXPDIS_DECOUPLED_GRAD_CKPT", "1") or "1").strip().lower() not in ("0", "false", "no")
    # Allow stacking: chunking + gradient checkpointing both reduce activation memory.
    # Override via EXPDIS_DECOUPLED_ALLOW_CHUNK_AND_CKPT=1 to keep grad_ckpt on when chunking.
    if use_token_chunk_train and _enable_grad_ckpt and not _env_flag("EXPDIS_DECOUPLED_ALLOW_CHUNK_AND_CKPT", default=True):
        ...
        if _xla_is_master_process(is_xla):
            print(
                ...
                f"(EXPDIS_DECOUPLED_TRAIN_TOKEN_CHUNK={token_chunk_size})",
                flush=True,
            )
    if _enable_grad_ckpt and not use_xla_fsdp:
        ...
        print("[decoupled-train] gradient checkpointing DISABLED (EXPDIS_DECOUPLED_GRAD_CKPT=0)")
    optim_override = (
        os.environ.get("EXPDIS_OPTIM", "").strip()
        or os.environ.get("EXPDIS_OPTIMIZER_OVERRIDE", "").strip()
        or "adamw_torch"
    )
    ...
    if optim_name in {"adamw", "adamw_torch", "adamw_hf"}:
        ...
        raise ValueError(
            f"Unsupported EXPDIS_OPTIM={optim_override!r} for decoupled Explorer training. "
    ...
    save_steps = max(0, int(os.environ.get("EXPDIS_GRPO_SAVE_STEPS", "0") or 0))
    log_history = []
    optimizer.zero_grad()
    micro_step = 0
    for batch_index, group_batch in enumerate(group_batches, start=1):
        ...
        if use_xla_fsdp or (fixed_shapes and fixed_completion_width > 0):
            shape_tag = f"expdis_decoupled_train_shape_s{global_step:06d}_m{micro_step:06d}"
            local_prompt_width = int(prompt_ids_cpu.size(1))
            local_completion_width = int(completion_ids_cpu.size(1))
            (
                prompt_ids_cpu,
        ...
        _flash_bf16 = is_xla and os.environ.get("EXPDIS_XLA_FLASH_ATTENTION", "0") not in ("0", "false", "no", "")
        for row_start in range(0, total_rows, row_microbatch_size):
            ...
            if use_token_chunk_train:
                ...
                if _flash_pad > 0 and is_xla and os.environ.get("EXPDIS_XLA_FLASH_ATTENTION", "0") not in ("0", "false", "no", ""):
                    _pad_ids = torch.full(
...
def train_explorer_with_decoupled_grpo_fsdp(
    ...
    *,
    ...
    cursor = 0
    base_round_groups = max(1, int(os.environ.get("EXPDIS_EXPLORER_DECOUPLED_BUFFER_GROUPS", "32") or 32))
    batch_groups = max(1, int(os.environ.get("EXPDIS_EXPLORER_DECOUPLED_BATCH_GROUPS", str(getattr(cfg, "grpo_batch_size", 1) or 1)) or 1))
    grad_accum = max(
        1,
        int(
    ...
    _xla_rendezvous("expdis_decoupled_fsdp_init_model_saved", is_xla=True)
    current_model = explorer_model.to(device)
    fsdp_grad_ckpt = _env_flag("EXPDIS_EXPLORER_DECOUPLED_TRAIN_FSDP_GRAD_CKPT", default=True)
    ...
    while global_step < total_steps:
        ...
        if is_master:
            ...
            del rollout_model
            gc.collect()
        _xla_rendezvous(f"expdis_decoupled_fsdp_rollout_ready_{int(global_step):06d}", is_xla=True)
        ...
        _xla_rendezvous(f"expdis_decoupled_fsdp_live_model_ready_{int(global_step):06d}", is_xla=True)
        ...
        round_counter += 1
        reload_every_rounds = int(os.environ.get("EXPDIS_EXPLORER_VLLM_RELOAD_EVERY_N_ROUNDS", "1") or 1)
        should_reload = (
            reload_every_rounds > 0
            and global_step < total_steps
            and (round_counter % reload_every_rounds == 0)
            and bool(os.environ.get("EXPDIS_EXPLORER_VLLM_RELOAD_TPU_NAME"))
        )
        if should_reload:
            try:
                from .pipeline import _save_model_to_gcs_and_reload_vllm
                run_name_env = os.environ.get("EXPDIS_WANDB_RUN_NAME", "").strip() or os.environ.get("WANDB_RUN_NAME", "").strip() or "explorer_run"
                reload_model = None
                if is_master:
                    print(
        ...
        _xla_rendezvous(f"expdis_decoupled_fsdp_reload_done_{int(global_step):06d}", is_xla=True)
...
def train_explorer_with_decoupled_grpo(
    ...
    *,
    ...
    if is_xla and os.environ.get("EXPDIS_FSDP", "1") == "0":
    ...
    if is_xla and world_size > 1:
        raise RuntimeError(
            ...
            "Use EXPDIS_EXPLORER_DECOUPLED_TRAIN_FSDP=1 for the multi-rank path or "
    ...
    random.Random(cfg.seed).shuffle(data_indices)
    cursor = 0
    buffer_groups_per_round = max(1, int(os.environ.get("EXPDIS_EXPLORER_DECOUPLED_BUFFER_GROUPS", "32") or 32))
    batch_groups = max(1, int(os.environ.get("EXPDIS_EXPLORER_DECOUPLED_BATCH_GROUPS", str(getattr(cfg, "grpo_batch_size", 1) or 1)) or 1))
    grad_accum = max(
        1,
        int(
    ...
    persistent_rollout_novelty_scorer = None
    os.makedirs(output_dir, exist_ok=True)
    # Resume from checkpoint if EXPDIS_RESUME_STEP is set.
    _resume_step = int(os.environ.get("EXPDIS_RESUME_STEP", "0") or 0)
    _resume_ckpt = os.environ.get("EXPDIS_RESUME_CHECKPOINT", "")
    if _resume_step > 0:
    ...
    while global_step < total_steps:
        ...
        _train_cpu = (os.environ.get("EXPDIS_DECOUPLED_TRAIN_CPU", "") or "").strip().lower() in ("1", "true", "yes")
...
def patch_grpo_trainer_generation_for_xla(trainer, answer_constraints=None):
    ...
    def _generate_and_score_completions_xla(self, inputs):
        ...
        use_vllm_for_grpo = _env_flag("EXPDIS_GRPO_USE_VLLM", default=False)
        vllm_urls_raw = (
            (os.environ.get("EXPDIS_VLLM_SERVER_URLS") or "").strip()
            if use_vllm_for_grpo
            else ""
        )
        if not vllm_urls_raw and (device.type != "xla" or os.environ.get("EXPDIS_XLA_SAFE_GENERATE", "1") == "0"):
        ...
        generation_debug = os.environ.get("EXPDIS_DEBUG_GENERATE", "0") == "1"
        if generation_debug:
            print(
        ...
        if vllm_urls_raw:
            ...
            with unwrap_model_for_generation(
                ...
                live_constraints = getattr(unwrapped_model, "_expdis_answer_constraints", None) or shared_constraints
                cfg_objs = (
        ...
        xla_cpu_handoff = (
            ...
            and _env_flag("EXPDIS_XLA_CPU_INPUT_HANDOFF", default=True)
        )
        if xla_cpu_handoff:
...
def train_explorer_with_grpo(train_ds,
    ...
    if is_xla and os.environ.get("EXPDIS_XLA_FIXED_PADDING", "1") != "0":
        patch_tokenizer_for_xla_fixed_padding(
            tokenizer,
            cfg.max_prompt_len,
    ...
    use_decoupled_xla_explorer = is_xla and _env_flag("EXPDIS_EXPLORER_DECOUPLED_GRPO", default=True)
    world_size = 1
    try:
    ...
    use_xla_world_size = is_xla and os.environ.get("EXPDIS_USE_XLA_WORLD_SIZE", "0") == "1"
    ...
    if is_xla:
        if use_xla_world_size:
            print(f"[xla] Using device world_size={world_size} (EXPDIS_USE_XLA_WORLD_SIZE=1).")
        else:
            print(
                f"[xla] Using process world_size={world_size} "
                "(set EXPDIS_USE_XLA_WORLD_SIZE=1 to use device count)."
            )
    ...
    if new_grad_accum != grad_accum:
        if use_decoupled_xla_explorer:
            ...
            strict_geometry = (os.environ.get("EXPDIS_STRICT_GRPO_GEOMETRY", "0") or "0").strip().lower() in (
                "1",
                "true",
                "yes",
            )
            if strict_geometry:
                raise ValueError(
                    ...
                    f"world_size={world_size}. Disable EXPDIS_STRICT_GRPO_GEOMETRY or change the "
    ...
    grad_ckpt_env = os.environ.get("EXPDIS_GRPO_GRAD_CKPT")
    ...
    fsdp_mode_env = os.environ.get("EXPDIS_FSDP", "").strip().lower()
    ...
    if is_xla and use_fsdp_full_shard and fsdp_grad_ckpt_requested:
        allow_unstable_fsdp_grad_ckpt = (
            os.environ.get("EXPDIS_XLA_ALLOW_UNSTABLE_FSDP_GRAD_CKPT", "0").strip().lower()
            in ("1", "true", "yes")
        )
    ...
    use_bf16 = (torch.cuda.is_available() or is_xla) and os.environ.get("EXPDIS_DISABLE_BF16", "0") != "1"
    if is_xla and not use_bf16:
        print("[patch] bf16 disabled for XLA GRPO via EXPDIS_DISABLE_BF16=1.")
    ...
    def _build_training_args(gen_batch_value: int) -> GRPOConfig:
        ...
        mask_truncated_env = os.environ.get("EXPDIS_MASK_TRUNCATED_COMPLETIONS")
        ...
        logging_strategy = str(os.environ.get("EXPDIS_GRPO_LOGGING_STRATEGY", "") or "").strip().lower() or "steps"
        logging_steps = max(1, int(os.environ.get("EXPDIS_GRPO_LOGGING_STEPS", "1") or 1))
        save_strategy = str(os.environ.get("EXPDIS_GRPO_SAVE_STRATEGY", "") or "").strip().lower() or "no"
        save_steps = max(1, int(os.environ.get("EXPDIS_GRPO_SAVE_STEPS", "8") or 8))
        save_total_limit = max(1, int(os.environ.get("EXPDIS_GRPO_SAVE_TOTAL_LIMIT", "2") or 2))
        if is_xla and save_strategy != "no" and os.environ.get("EXPDIS_ALLOW_XLA_GRPO_INTERMEDIATE_SAVE", "0") not in ("1", "true", "True", "yes", "YES"):
        ...
        beta_env = os.environ.get("EXPDIS_GRPO_BETA", "").strip()
        ...
        if beta_val is not None and "beta" in supported_args:
            ...
            print(f"[grpo] KL beta={beta_val} (EXPDIS_GRPO_BETA)")
        optim_override = os.environ.get("EXPDIS_OPTIM", "").strip() or os.environ.get("EXPDIS_OPTIMIZER_OVERRIDE", "").strip()
    ...
    if is_xla:
        xla_train_top_p = float(
            os.environ.get(
                "EXPDIS_EXPLORER_TRAIN_TOP_P",
                os.environ.get("EXPDIS_EXPLORER_VLLM_TOP_P", "0.95"),
            )
            or 0.95
        )
        xla_train_top_k = int(
            os.environ.get(
                "EXPDIS_EXPLORER_TRAIN_TOP_K",
                os.environ.get("EXPDIS_EXPLORER_VLLM_TOP_K", "20"),
            )
            or 20
        )
    ...
    if answer_constraints is not None:
        explorer_model._expdis_answer_constraints = answer_constraints
    ...
    if is_xla:
        ...
        use_safe_generate = os.environ.get("EXPDIS_XLA_SAFE_GENERATE", "1") != "0"
    ...
    _use_decoupled = (
        is_xla
        or _env_flag("EXPDIS_DECOUPLED_TRAIN_CPU", default=False)
        or _env_flag("EXPDIS_FORCE_DECOUPLED", default=False)
    )
    if _use_decoupled and _env_flag("EXPDIS_EXPLORER_DECOUPLED_GRPO", default=True):
        if (
            _env_flag("EXPDIS_EXPLORER_DECOUPLED_TRAIN_FSDP", default=_env_flag("EXPDIS_FSDP", default=False))
            and _xla_runtime_rank_world()[1] > 1
        ):
    ...
    vllm_active = bool((os.environ.get("EXPDIS_VLLM_SERVER_URLS") or "").strip())
    if vllm_active or (is_xla and os.environ.get("EXPDIS_XLA_SAFE_GENERATE", "1") != "0"):
    ...
    if os.environ.get("EXPDIS_RESUME_TRAINING", "0") not in ("", "0", "false", "False", "no", "NO"):
    ...
    if _env_flag("EXPDIS_ABORT_ON_DEGENERATE_EXPLORER", default=True):
        terminal_row = {}
...
def generate_offline_trajectories(explorer_model,
    ...
    traj_max_override = (os.environ.get("EXPDIS_TRAJ_MAX_COMPLETION_LEN") or "").strip()
    if traj_max_override:
        max_completion_len = int(traj_max_override)
    health_window_prompts = max(0, int(os.environ.get("EXPDIS_TRAJ_HEALTH_WINDOW_PROMPTS", "32") or 32))
    health_min_correct = max(0, int(os.environ.get("EXPDIS_TRAJ_HEALTH_MIN_CORRECT", "1") or 1))
    accepted_target_raw = str(os.environ.get("EXPDIS_TRAJ_TARGET_ACCEPTED", "") or "").strip()
    ...
    for temp in temps:
        for start in tqdm(range(0, len(train_ds), trajectory_batch_size), desc=f"Collecting Explorer trajectories (temp={temp})"):
            ...
            # has been reloaded with the Explorer model's weights (EXPDIS_TRAJ_USE_VLLM=1).
            # By default, vLLM serves BASE weights which are NOT the trained Explorer.
            traj_use_vllm = os.environ.get("EXPDIS_TRAJ_USE_VLLM", "0").strip() == "1"
            vllm_urls_raw = (os.environ.get("EXPDIS_VLLM_SERVER_URLS") or "").strip() if traj_use_vllm else ""
            if vllm_urls_raw:
                from expdis_torch.vllm_generate import generate_completions_vllm
                vllm_urls = [u.strip() for u in vllm_urls_raw.split(",") if u.strip()]
                vllm_model = os.environ.get("EXPDIS_VLLM_MODEL_NAME", "")
                vllm_api_key = os.environ.get("EXPDIS_VLLM_API_KEY", "")
                vllm_concurrency = int(os.environ.get("EXPDIS_VLLM_CONCURRENCY", "64"))
                vllm_top_p = float(os.environ.get("EXPDIS_TRAJ_VLLM_TOP_P", os.environ.get("EXPDIS_EXPLORER_VLLM_TOP_P", "0.95")) or 0.95)
                vllm_top_k = int(os.environ.get("EXPDIS_TRAJ_VLLM_TOP_K", os.environ.get("EXPDIS_EXPLORER_VLLM_TOP_K", "20")) or 20)
                thinking_mode = _resolved_thinking_mode(cfg, phase="explore")
                vllm_enable_thinking = True if thinking_mode == "think" else (False if thinking_mode == "no_think" else None)
                all_completions = generate_completions_vllm(
                    prompts=model_prompts,
            ...
            if (
                _env_flag("EXPDIS_ABORT_ON_BAD_TRAJ_WINDOW", default=True)
