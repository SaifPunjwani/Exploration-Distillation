...
def evaluate_model_on_gsm8k(model, tokenizer, eval_ds, cfg, novelty_scorer, desc="Model", use_wandb: bool = False, wandb_run=None):
    ...
    vllm_urls_raw = (os.environ.get("EXPDIS_VLLM_SERVER_URLS") or "").strip()
    # Use vLLM for Base eval (servers serve base weights).
    # For Actual eval, use EXPDIS_ACTUAL_VLLM_SERVER_URL if set (a server reloaded with Actual weights).
    actual_vllm_url = (os.environ.get("EXPDIS_ACTUAL_VLLM_SERVER_URL") or "").strip()
...
def _prefer_native_xla_eval_generate(answer_constraints, *, do_sample: bool = False) -> bool:
    ...
    if answer_constraints is not None:
        return False
    flag = str(os.environ.get("EXPDIS_XLA_EVAL_NATIVE_GENERATE", "1") or "1").strip().lower()
