...
def _stage_resume_enabled() -> bool:
    return os.environ.get("EXPDIS_RESUME_TRAINING", "0") not in ("", "0", "false", "False", "no", "NO")
...
def _save_periodic_step_checkpoint(model, tokenizer, cfg, stage_name: str, global_step: int):
    save_steps = max(0, int(os.environ.get("EXPDIS_ACTUAL_GRPO_SAVE_STEPS", "0") or 0))
    if save_steps <= 0 or global_step <= 0 or (global_step % save_steps) != 0:
        return
    save_limit = max(1, int(os.environ.get("EXPDIS_ACTUAL_GRPO_SAVE_TOTAL_LIMIT", "4") or 4))
...
def _resolve_training_world_size(is_xla: bool) -> int:
    ...
    if world_size <= 0:
        world_size = 1
    if is_xla and os.environ.get("EXPDIS_USE_XLA_WORLD_SIZE", "0") == "1":
        try:
...
def train_actual_model(distill_dataset, tokenizer, cfg, device, is_xla: bool, use_wandb: bool, wandb_run=None, init_model=None):
    ...
    # Control via EXPDIS_DISTILL_EAGER_ATTN: "1" force eager, "0" keep flash,
    ...
    _force_eager = os.environ.get("EXPDIS_DISTILL_EAGER_ATTN", "").strip().lower()
    if _distill_orig_attn == "tpu_flash" and _force_eager in ("1", "true", "yes"):
        ...
        print("[distill] Flash attn → eager (EXPDIS_DISTILL_EAGER_ATTN=1)")
    ...
    grad_ckpt_env = (os.environ.get("EXPDIS_DISTILL_GRAD_CKPT", "1" if is_xla else "0") or "0").strip().lower()
    ...
    optim_override = (
        os.environ.get("EXPDIS_DISTILL_OPTIM", "").strip()
        or os.environ.get("EXPDIS_OPTIM", "").strip()
        or "adamw_torch"
    ).lower()
    if optim_override in {"adamw", "adamw_torch", "adamw_hf"}:
        ...
        raise ValueError(
            f"Unsupported EXPDIS_DISTILL_OPTIM/EXPDIS_OPTIM={optim_override!r}. "
    ...
    for epoch in range(start_epoch, cfg.distill_epochs):
        ...
        for batch in tqdm(loader, desc=f"Distill epoch {epoch+1}"):
            ...
            _CHUNK = max(64, int(os.environ.get("EXPDIS_DISTILL_CE_CHUNK", "512") or 512))
...
def train_actual_with_offline_grpo(actual_dataset,
    ...
    optim_override = (
        os.environ.get("EXPDIS_ACTUAL_GRPO_OPTIM", "").strip()
        or os.environ.get("EXPDIS_OPTIM", "").strip()
        or "adamw_torch"
    ).lower()
    if optim_override in {"adamw", "adamw_torch", "adamw_hf"}:
...
def train_actual_with_online_grpo(train_ds, tokenizer, cfg, device, is_xla,
    ...
    save_steps_env = os.environ.get("EXPDIS_ACTUAL_GRPO_SAVE_STEPS", "25")
    save_limit_env = os.environ.get("EXPDIS_ACTUAL_GRPO_SAVE_TOTAL_LIMIT", "4")
    prev_save_steps = os.environ.get("EXPDIS_GRPO_SAVE_STEPS")
    prev_save_limit = os.environ.get("EXPDIS_GRPO_SAVE_TOTAL_LIMIT")
    os.environ["EXPDIS_GRPO_SAVE_STEPS"] = save_steps_env
    os.environ["EXPDIS_GRPO_SAVE_TOTAL_LIMIT"] = save_limit_env

    # Reset Explorer resume state so Actual GRPO starts from step 0,
    # unless EXPDIS_ACTUAL_GRPO_RESUME_STEP / EXPDIS_ACTUAL_GRPO_RESUME_CHECKPOINT
    # are set for resuming Actual GRPO from a checkpoint.
    prev_resume_step = os.environ.get("EXPDIS_RESUME_STEP")
    prev_resume_ckpt = os.environ.get("EXPDIS_RESUME_CHECKPOINT")
    actual_resume_step = os.environ.get("EXPDIS_ACTUAL_GRPO_RESUME_STEP", "0")
    actual_resume_ckpt = os.environ.get("EXPDIS_ACTUAL_GRPO_RESUME_CHECKPOINT", "")
    os.environ["EXPDIS_RESUME_STEP"] = actual_resume_step
    if actual_resume_ckpt:
        os.environ["EXPDIS_RESUME_CHECKPOINT"] = actual_resume_ckpt

    # Wire actual_grpo_kl_coef through to decoupled training path via env var
    prev_grpo_beta = os.environ.get("EXPDIS_GRPO_BETA")
    kl_coef = getattr(cfg, "actual_grpo_kl_coef", 0.0) or 0.0
    if kl_coef > 0:
        os.environ["EXPDIS_GRPO_BETA"] = str(kl_coef)
    ...
    try:
        ...
        if prev_resume_step is not None:
            os.environ["EXPDIS_RESUME_STEP"] = prev_resume_step
        elif "EXPDIS_RESUME_STEP" in os.environ:
            del os.environ["EXPDIS_RESUME_STEP"]
        if prev_resume_ckpt is not None:
            os.environ["EXPDIS_RESUME_CHECKPOINT"] = prev_resume_ckpt
        elif "EXPDIS_RESUME_CHECKPOINT" in os.environ:
            del os.environ["EXPDIS_RESUME_CHECKPOINT"]
        if prev_save_steps is not None:
            os.environ["EXPDIS_GRPO_SAVE_STEPS"] = prev_save_steps
        elif "EXPDIS_GRPO_SAVE_STEPS" in os.environ:
            del os.environ["EXPDIS_GRPO_SAVE_STEPS"]
        if prev_save_limit is not None:
            os.environ["EXPDIS_GRPO_SAVE_TOTAL_LIMIT"] = prev_save_limit
        elif "EXPDIS_GRPO_SAVE_TOTAL_LIMIT" in os.environ:
            del os.environ["EXPDIS_GRPO_SAVE_TOTAL_LIMIT"]
        if prev_grpo_beta is not None:
            os.environ["EXPDIS_GRPO_BETA"] = prev_grpo_beta
        elif "EXPDIS_GRPO_BETA" in os.environ:
            del os.environ["EXPDIS_GRPO_BETA"]
