...
def _save_model_to_gcs_and_reload_vllm(model, tokenizer, run_name: str, role: str = "actual"):
    """Save model to GCS and optionally reload a vLLM server to serve it.

    Returns the GCS model path if successful, or None.
    """
    ...
    barrier_tag = f"expdis_ckpt_{_artifact_safe_name(run_name)}_{str(role).lower()}"
    ...
    if not gcs_base:
        gcs_base = "gs://llmvae-checkpoints/two-model-exploration/checkpoints"

    gcs_model_path = f"{gcs_base}/{run_name}/{role}"
    local_tmp = f"/tmp/expdis_model_save_{_artifact_safe_name(run_name)}_{str(role).lower()}_{os.getpid()}"

    try:
        print(f"[ckpt] Saving {role} model to {gcs_model_path}...")
        ...
        try:
            ...
            if ret.returncode == 0:
                print(f"[ckpt] Saved {role} model to {gcs_model_path}")
            else:
                print(f"[ckpt] WARNING: gsutil upload failed: {ret.stderr[:200]}")
                return None
        except Exception as e:
            print(f"[ckpt] WARNING: gsutil upload error: {e}")
            return None
        ...
        if role == "explorer":
            reload_tpu_name = os.environ.get("EXPDIS_EXPLORER_VLLM_RELOAD_TPU_NAME", "").strip()
            reload_zone = os.environ.get("EXPDIS_EXPLORER_VLLM_RELOAD_ZONE", "").strip()
            reload_workers = os.environ.get("EXPDIS_EXPLORER_VLLM_RELOAD_WORKERS", "all").strip() or "all"
            ...
            # with EXPDIS_EXPLORER_VLLM_MODEL_NAME if you deliberately want a new id.
            _client_current = os.environ.get("EXPDIS_VLLM_MODEL_NAME", "").strip()
            served_model_name = (
                os.environ.get("EXPDIS_EXPLORER_VLLM_MODEL_NAME", "").strip()
                or _client_current
                or "explorer_model"
            )
            ...
            if reload_tpu_name and reload_zone:
                print(
                    ...
                    f"(workers={reload_workers}) from {gcs_model_path}..."
                )
            ...
            if reload_succeeded:
                os.environ["EXPDIS_TRAJ_USE_VLLM"] = "1"
                os.environ["EXPDIS_VLLM_MODEL_NAME"] = served_model_name
                print(
                    f"[ckpt] Enabled TPU-backed vLLM trajectory collection with "
                    f"EXPDIS_VLLM_MODEL_NAME={served_model_name}"
                )
        ...
        vllm_reload_url = os.environ.get("EXPDIS_VLLM_RELOAD_WORKER_URL", "").strip()
        if role == "actual" and vllm_reload_url:
            reload_tpu_name = os.environ.get("EXPDIS_VLLM_RELOAD_TPU_NAME", "").strip()
            reload_zone = os.environ.get("EXPDIS_VLLM_RELOAD_ZONE", "").strip()
            reload_worker = os.environ.get("EXPDIS_VLLM_RELOAD_WORKER", "0").strip() or "0"
            served_model_name = os.environ.get("EXPDIS_ACTUAL_VLLM_MODEL_NAME", "actual_model").strip() or "actual_model"
            reload_succeeded = False
            if reload_tpu_name and reload_zone:
                print(
                    f"[ckpt] Reloading vLLM worker {reload_worker} on {reload_tpu_name} "
                    f"from {gcs_model_path}..."
                )
                reload_env = os.environ.copy()
                reload_env.update(
                    {
                ...
                print(
                    "[ckpt] WARNING: EXPDIS_VLLM_RELOAD_WORKER_URL set without "
                    "EXPDIS_VLLM_RELOAD_TPU_NAME/EXPDIS_VLLM_RELOAD_ZONE; skipping remote reload."
                )

            if reload_succeeded:
                os.environ["EXPDIS_ACTUAL_VLLM_MODEL_PATH"] = gcs_model_path
                os.environ["EXPDIS_ACTUAL_VLLM_SERVER_URL"] = vllm_reload_url
                print(f"[ckpt] Set EXPDIS_ACTUAL_VLLM_SERVER_URL={vllm_reload_url} for Actual eval")
...
def _wandb_enabled_for_this_process() -> bool:
    if os.environ.get("EXPDIS_WANDB_MASTER_ONLY", "1") == "0":
        return True
...
def _resolve_final_eval_roles() -> set[str]:
    if _env_flag("EXPDIS_FINAL_EVAL_ONLY_ACTUAL", "0"):
        return {"actual"}
    raw = str(os.environ.get("EXPDIS_FINAL_EVAL_ROLES", "") or "").strip().lower()
    if not raw:
...
def _resolve_final_eval_cfg(cfg: RunConfig) -> RunConfig:
    final_cfg = cfg
    max_eval_override = str(os.environ.get("EXPDIS_FINAL_EVAL_MAX_EXAMPLES", "") or "").strip()
    if max_eval_override:
        try:
            parsed = max(0, int(max_eval_override))
            final_cfg = replace(final_cfg, max_eval_examples=parsed)
        except Exception:
            print(f"[WARN] Ignoring invalid EXPDIS_FINAL_EVAL_MAX_EXAMPLES={max_eval_override!r}")

    max_benchmark_override = str(os.environ.get("EXPDIS_FINAL_BENCHMARK_MAX_EXAMPLES", "") or "").strip()
    if max_benchmark_override:
        try:
            ...
            print(f"[WARN] Ignoring invalid EXPDIS_FINAL_BENCHMARK_MAX_EXAMPLES={max_benchmark_override!r}")
    return final_cfg
...
def _run_training_round(
    ...
    skip_intermediate_evals = str(os.environ.get("EXPDIS_SKIP_INTERMEDIATE_EVALS", "0") or "0").strip().lower() in (
        "1",
        "true",
        "yes",
    )
    ...
    _precomp_path = os.environ.get("EXPDIS_PRECOMPUTED_TRAJ_PATH", "").strip()
    ...
    elif (explorer_pretrained_path := os.environ.get("EXPDIS_EXPLORER_PRETRAINED_PATH", "").strip()):
    ...
    if _env_flag("EXPDIS_STOP_AFTER_EXPLORER", "0"):
    ...
    precomputed_traj_path = os.environ.get("EXPDIS_PRECOMPUTED_TRAJ_PATH", "").strip()
    ...
    # Back up trajectories to GCS
    _run_name = getattr(cfg, "wandb_run_name", None) or "unnamed"
    _gcs_base = os.environ.get("EXPDIS_GCS_CHECKPOINT_BASE", "").strip()
    if not _gcs_base:
        _gcs_base = "gs://llmvae-checkpoints/two-model-exploration/checkpoints"
    _traj_local = getattr(cfg, "explorer_traj_path", None)
    if _traj_local and os.path.exists(str(_traj_local)):
        _gcs_traj = f"{_gcs_base}/{_run_name}/explorer_trajectories.jsonl"
        try:
            _ret = subprocess.run(
            ...
            if _ret.returncode == 0:
                print(f"[ckpt] Backed up trajectories to {_gcs_traj}")
            else:
                print(f"[ckpt] WARNING: trajectory backup failed: {_ret.stderr[:200]}")
        except Exception as _e:
            print(f"[ckpt] WARNING: trajectory backup error: {_e}")

    if _env_flag("EXPDIS_STOP_AFTER_TRAJECTORIES", "0"):
    ...
    if cfg.decoupling_mode == "decoupled":
        ...
        if cfg.actual_train_mode == "distill":
            if not actual_gate["skip"]:
                ...
                _distill_max_len = int(os.environ.get("EXPDIS_DISTILL_MAX_TOTAL_LEN", "0") or 0) or cfg.max_total_len
            ...
            if actual_gate["skip"]:
                ...
                if cfg.actual_train_mode == "sft_online_grpo":
                    ...
                    _actual_resume_ckpt = os.environ.get("EXPDIS_ACTUAL_GRPO_RESUME_CHECKPOINT", "")
    ...
    skip_final_benchmark_evals = _env_flag("EXPDIS_SKIP_FINAL_BENCHMARK_EVALS", "0")
...
def run_pipeline(cfg: RunConfig):
    ...
    if is_xla:
        ...
        patch_torch_xla_device_module_for_rng()
        patch_torch_checkpoint_autocast_for_xla()
        if os.environ.get("EXPDIS_XLA_FLASH_ATTENTION", "0") not in ("0", "false", "no", ""):
            _register_tpu_flash_attention()
        patch_torch_isin_for_xla()
        patch_transformers_isin_for_xla()
        patch_transformers_attention_mask_for_xla()
