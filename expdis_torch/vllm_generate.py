...
def generate_completions_vllm(
    ...
    # doesn't cause empty completions. Set EXPDIS_VLLM_MAX_RETRIES to limit.
    max_retries_raw = (os.environ.get("EXPDIS_VLLM_MAX_RETRIES") or "").strip()
...
def discover_vllm_servers(
    ...
    if worker_ips is None:
        endpoints_raw = os.environ.get("EXPDIS_VLLM_WORKER_IPS", "").strip()
        if endpoints_raw:
