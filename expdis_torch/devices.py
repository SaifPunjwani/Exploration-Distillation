...
def _allow_single_worker_slice_override() -> bool:
    if os.environ.get("EXPDIS_ALLOW_SINGLE_WORKER_TPU") == "1":
        return True
...
def clear_torch_xla_env():
    if os.environ.get("EXPDIS_TPU_SLICE_LAUNCHED") == "1" and os.environ.get("EXPDIS_REQUIRE_TPU") == "1":
...
def maybe_import_xla():
    ...
    try:
        ...
        if current_mark_step is not None and not getattr(current_mark_step, "_expdis_env_guard", False):
            ...
            mark_step_env_guard._expdis_env_guard = True  # type: ignore[attr-defined]
...
def resolve_device(preference: str = "auto"):
    ...
    if (
        ...
        and os.environ.get("EXPDIS_TPU_SLICE_LAUNCHED") != "1"
        and not _allow_single_worker_slice_override()
    ):
        msg = _slice_launch_guard_message()
        if pref == "tpu":
            if os.environ.get("EXPDIS_REQUIRE_TPU") == "1":
    ...
    if use_xla:
        ...
        if xm_mod is not None and xr_mod is not None:
            try:
                ...
                if os.environ.get("EXPDIS_REQUIRE_TPU") == "1" and pref == "tpu":
                    raise RuntimeError(
                        "[devices] TPU initialization failed in a slice-launched run. "
                        "Refusing CPU fallback because EXPDIS_REQUIRE_TPU=1."
                    ) from e
        clear_torch_xla_env()
    elif pref == "tpu":
        ...
        if os.environ.get("EXPDIS_REQUIRE_TPU") == "1":
            raise RuntimeError(f"{msg} Refusing to fall back because EXPDIS_REQUIRE_TPU=1.")
        print(f"{msg} Falling back to CPU.")
