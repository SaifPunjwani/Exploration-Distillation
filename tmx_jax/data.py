"""DAPO-Math-17k dataset loader + reward extraction.

Uses HF datasets. Each row: {"problem": str, "solution": str, "answer": str}.
We apply the model chat template via the tokenizer/processor so vLLM receives a properly
formatted prompt (no double-templating: we pass the raw prompt string to
vLLM's /completions, not /chat/completions).
"""
...
def training_dataset_identity(dataset_name: str) -> Dict[str, Any]:
    ...
    jsonl_path = os.environ.get("TMX_TRAIN_DATASET_JSONL", "").strip()
...
def _round_shard_required() -> bool:
    return (os.environ.get("TMX_DATASET_SHARD_REQUIRED", "") or "").strip().lower() \
        in ("1", "true", "yes")
...
def _round_shard_settings() -> Tuple[int, int, str, int, bool]:
    ...
    names = (
        "TMX_DATASET_SHARD_INDEX", "TMX_DATASET_SHARD_COUNT",
        "TMX_DATASET_SHARD_POLICY", "TMX_DATASET_SHARD_SEED",
    )
    if required:
    ...
    if shard_count < 1:
        raise ValueError(
            f"TMX_DATASET_SHARD_COUNT must be >= 1, got {shard_count}"
        )
    ...
    if shard_index < 0 or shard_index >= shard_count:
        raise ValueError(
            f"TMX_DATASET_SHARD_INDEX must be in [0, {shard_count}), got {shard_index}"
        )
    policy = (
        os.environ.get("TMX_DATASET_SHARD_POLICY", "uniform") or "uniform"
    ).strip().lower()
    ...
    if policy not in ("uniform", "sequential"):
        raise ValueError(
            "unsupported TMX_DATASET_SHARD_POLICY="
    ...
    seed = int(os.environ.get("TMX_DATASET_SHARD_SEED", "0") or "0")
...
def _write_or_verify_round_shard_manifest(
    ...
    if not target:
        raise ValueError("TMX_DATASET_SHARD_MANIFEST_PATH is empty")
...
def build_round_shard(
    ...
    manifest_path = (
        os.environ.get("TMX_DATASET_SHARD_MANIFEST_PATH", "") or ""
    ).strip()
    if required and not manifest_path:
        raise ValueError(
            "MR-ME requires TMX_DATASET_SHARD_MANIFEST_PATH so every Scout "
...
def _apply_round_shard(rows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    The pipeline sets TMX_DATASET_SHARD_* per round. Sharding happens after the
...
def extract_boxed(text: str) -> Optional[str]:
    """Return last \\boxed{...} content, or None."""
...
def is_correct(completion_text: str, ground_truth: str) -> bool:
    pred = extract_boxed(completion_text)
    if pred is None:
        return False
    return normalize_answer(pred) == normalize_answer(ground_truth)


def render_chat_prompt(problem: str, tokenizer, enable_thinking: bool = True) -> str:
...
def render_qwen3_prompt(problem: str, tokenizer, enable_thinking: bool = True) -> str:
    """Backward-compatible alias for older call sites."""
    return render_chat_prompt(problem, tokenizer, enable_thinking=enable_thinking)
...
def load_examples(dataset_name: str, tokenizer, max_examples: int | None = None,
    ...
    if name.startswith("jsonl:"):
        ...
        # the actual file path through TMX_TRAIN_DATASET_JSONL.
...
def compute_reward(completion_text: str, ground_truth: str,
                   reward_correct: float = 1.0, reward_incorrect: float = -1.0,
                   format_penalty: float = 0.1) -> Tuple[float, bool]:
    """Return (reward, is_correct).

    reward = reward_correct (correct) OR reward_incorrect - format_penalty (no boxed)
                                     OR reward_incorrect (boxed but wrong).
    Production DAPO/ExpDis uses reward_correct=+1 and reward_incorrect=-1.
    """
    pred = extract_boxed(completion_text or "")
    if pred is None:
        return reward_incorrect - format_penalty, False
    correct = normalize_answer(pred) == normalize_answer(ground_truth)
    return (reward_correct if correct else reward_incorrect), correct
