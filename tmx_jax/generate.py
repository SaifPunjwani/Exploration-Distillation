...
Ported from `tmx/vllm_generate.py`; this version is pure-Python (no torch).
Generation returns raw text + per-token logprobs so we can recompute
logprobs on the training policy (the anti-stale-vLLM fix).
"""
...
# Module-level Session with connection pooling. Cross-region vLLM (us-central
# training -> europe-west4 serving) RTT is ~80ms; without keep-alive every
...
_HTTP_POOL_MAXSIZE = max(128, _env_int("TMX_HTTP_POOL_MAXSIZE", 512))
...
def _filter_live_servers(server_urls: List[str]) -> List[str]:
    ...
    enabled = os.environ.get("TMX_VLLM_FILTER_DEAD_SERVERS", "1").strip().lower()
    if enabled in ("0", "false", "no", "off") or len(server_urls) <= 1:
        return server_urls
    timeout = float(os.environ.get("TMX_VLLM_HEALTH_FILTER_TIMEOUT", "2") or "2")
    ttl = float(os.environ.get("TMX_VLLM_HEALTH_FILTER_TTL_SECONDS", "30") or "30")
    concurrency = max(1, min(len(server_urls), _env_int("TMX_VLLM_HEALTH_FILTER_CONCURRENCY", 64)))
...
def _vllm_complete(
    ...
    seed_mode = os.environ.get("TMX_VLLM_SEED_MODE", "api").strip().lower()
    request_prompt = prompt
    if seed is not None and seed_mode in ("prompt_nonce", "whitespace_nonce"):
        # Some TPU vLLM OpenAI-compatible endpoints reject the non-standard
        # `seed` field. A tiny whitespace suffix perturbs the prefill enough to
        # avoid identical fanout RNG paths without changing the math problem.
        request_prompt = prompt + ("\n" * (1 + (int(seed) % 3))) + (" " * (1 + (int(seed) % 17)))
    payload = {
        "model": model,
    ...
    if (os.environ.get("TMX_VLLM_RETURN_LOGPROBS", "0") or "").strip().lower() not in ("0", "false", "no", "off"):
        payload["logprobs"] = 1
    if enable_thinking in ("always", "never"):
        # vLLM lifted Qwen-specific "enable_thinking" via extra_body; be permissive.
        payload["extra_body"] = {"chat_template_kwargs": {"enable_thinking": enable_thinking == "always"}}
    ...
    for attempt in range(max_retries + 1):
        try:
            ...
            if (
                ...
                and "seed" in payload
                and os.environ.get("TMX_VLLM_SEED_API_FALLBACK", "1").strip().lower()
...
def batched_generate(
    ...
    if allow_error_completions is None:
        allow_error_completions = (
            os.environ.get("TMX_VLLM_ALLOW_ERROR_COMPLETIONS", "0").strip().lower()
            in ("1", "true", "yes", "on")
        )
    backend = os.environ.get("TMX_ROLLOUT_BACKEND", "adaptive_vllm_http").strip().lower()
    if backend not in ("vllm_http", "adaptive_vllm_http"):
        raise ValueError(
            f"Unsupported TMX_ROLLOUT_BACKEND={backend!r}. "
    ...
    endpoint_max_inflight = max(1, _env_int("TMX_VLLM_ENDPOINT_MAX_INFLIGHT", 1))
    failover_rounds = max(1, _env_int("TMX_VLLM_FAILOVER_ROUNDS", 2))
    failover_round_sleep = max(0.0, float(os.environ.get("TMX_VLLM_FAILOVER_ROUND_SLEEP", "10") or "10"))
    failover_on_error = (
        os.environ.get("TMX_VLLM_FAILOVER_ON_ERROR", "1").strip().lower()
        not in ("0", "false", "no", "off")
    )
    randomize_seeds = (
        os.environ.get("TMX_VLLM_RANDOMIZE_SEEDS", "0").strip().lower()
        not in ("0", "false", "no", "off")
    )
    seed_base_env = os.environ.get("TMX_VLLM_SEED_BASE", "").strip()
    if seed_base_env:
        try:
    ...
    if not _PRINTED_BACKEND_CONFIG:
        ...
        print(
            ...
            f"health_ttl={os.environ.get('TMX_VLLM_HEALTH_FILTER_TTL_SECONDS', '30')} "
    ...
    def _seed_for(prompt_idx: int, split_idx: int, count: int) -> Optional[int]:
        if not randomize_seeds:
            return None
        # Stable if TMX_VLLM_SEED_BASE is provided; distinct across prompts and
