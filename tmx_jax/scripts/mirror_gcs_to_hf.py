...
def main() -> int:
    ...
    p.add_argument("--repo-id", default=os.environ.get("TMX_HF_CHECKPOINT_REPO", "SaifPunjwani/two-model-exploration-checkpoints"))
    ...
    failures = 0
    with tempfile.TemporaryDirectory(prefix="tmx_gcs_to_hf_") as tmp:
        tmpdir = Path(tmp)
