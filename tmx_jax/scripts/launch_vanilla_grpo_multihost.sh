...
#   TPU_NAME=<tpu-name> \
#   ZONE=us-central1-a \
#   bash tmx_jax/scripts/launch_vanilla_grpo_multihost.sh
...
export TMX_JAX_CLUSTER_DETECTION_METHOD='deactivate'
...
export TMX_HF_MIRROR_CHECKPOINTS=1
export TMX_HF_MIRROR_TRAJECTORIES=1
export TMX_HF_MIRROR_METRICS=1
export TMX_HF_MIRROR_VLLM_EXPORTS=1
export TMX_HF_MIRROR_EVAL_PROBES=1
export TMX_HF_MIRROR_RUN_SUMMARY=1
export TMX_HF_MIRROR_FINAL_EXPORTS=1
export TMX_HF_MIRROR_FINAL_EVAL=1
export TMX_HF_MIRROR_ASYNC=1
export TMX_ALLOW_GCS_ARTIFACTS=0
export TMX_GCS_CHECKPOINT_BASE=''
export TMX_REQUIRE_GCS_CHECKPOINT_UPLOAD=0
export TMX_SAVE_BASE_POLICY_TRAJECTORIES=0
...
INNER
  ...
  if [ "$SYNC_LOCAL_PACKAGE" = "1" ]; then
    gcloud --project="$PROJECT" alpha compute tpus tpu-vm scp --recurse \
      "$LOCAL_REPO_ROOT/tmx_jax" "$TPU_NAME:${REMOTE_REPO}/" \
  ...
  gcloud --project="$PROJECT" alpha compute tpus tpu-vm ssh "$TPU_NAME" \
    ...
    pkill -9 -f '[p]ython -m tmx_jax.train' 2>/dev/null || true
