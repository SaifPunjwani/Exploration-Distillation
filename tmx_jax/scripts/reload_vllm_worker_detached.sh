...
rm -rf /tmp/tmx_external_*_w"${WORKER_ID}" /tmp/tmx_vllm_reload_* 2>/dev/null || true
rm -rf "$STAGING_DIR"
mkdir -p "$STAGING_DIR"
...
with tempfile.NamedTemporaryFile(prefix="tmx_vllm_reload_", suffix=".tar", delete=False) as f:
    tmp = Path(f.name)
try:
