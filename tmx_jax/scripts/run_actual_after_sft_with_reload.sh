...
REPO_ROOT="${REPO_ROOT:-$HOME/tmx-jax}"
...
if [ -z "$VLLM_URLS" ]; then
  echo "[actual-resume] ERROR: set TMX_VLLM_SERVER_URLS or VLLM_SERVER_URLS" >&2
  exit 2
fi
if [ ! -d "$SFT_CKPT" ]; then
  if [ "${TMX_ALLOW_GCS_ARTIFACTS:-0}" != "1" ] || [ -z "$SFT_CKPT_GCS" ]; then
...
cd "$REPO_ROOT"
source "${ENV_DIR:-$HOME/miniforge3}/bin/activate" "${ENV_NAME:-tmx_jax}"
mkdir -p "$ACTUAL_OUT"
INIT_CKPT="${INIT_CKPT:-}"
if [ -z "$INIT_CKPT" ]; then
...
export TMX_HF_CHECKPOINT_REPO="$TMX_HF_CHECKPOINT_REPO"
export TMX_HF_REPO_TYPE="$TMX_HF_REPO_TYPE"
export TMX_GCS_CHECKPOINT_BASE="${TMX_GCS_CHECKPOINT_BASE:-}"
export TMX_SAVE_OPT_STATE="${TMX_SAVE_OPT_STATE:-0}"
export TMX_RESTORE_OPT_STATE="${TMX_RESTORE_OPT_STATE:-0}"
...
export TMX_GCS_CHECKPOINT_EVERY_STEPS="${TMX_GCS_CHECKPOINT_EVERY_STEPS:-${SAVE_EVERY_STEPS:-50}}"
...
export TMX_TRAIN_VLLM_RELOAD_EVERY_STEPS="${TMX_TRAIN_VLLM_RELOAD_EVERY_STEPS:-${SAVE_EVERY_STEPS:-50}}"
...
reload_workers="${TMX_VLLM_RELOAD_WORKERS:-}"
if [ -z "$reload_workers" ]; then
  echo "[actual-resume] ERROR: set TMX_VLLM_RELOAD_WORKERS" >&2
  exit 2
fi

attempt=0
if [ "${TMX_ACTUAL_ASSUME_VLLM_RELOADED:-0}" = "1" ]; then
  ...
  while true; do
    ...
    if env \
      ...
      TPU_NAME="${TMX_VLLM_RELOAD_TPU_NAME:?Must set TMX_VLLM_RELOAD_TPU_NAME}" \
      ZONE="${TMX_VLLM_RELOAD_ZONE:?Must set TMX_VLLM_RELOAD_ZONE}" \
      WORKERS="$reload_workers" \
      SERVED_MODEL_NAME="$MODEL_NAME" \
      LOCAL_MODEL_DIR_BASENAME="${TMX_ACTUAL_SFT_LOCAL_MODEL_BASENAME:-tmx_actual_sft_model}" \
      MAX_MODEL_LEN="${TMX_ACTUAL_GRPO_MAX_MODEL_LEN:-$MAX_TOTAL_LEN}" \
      MAX_NUM_SEQS="${TMX_ACTUAL_GRPO_VLLM_MAX_NUM_SEQS:-32}" \
      MAX_NUM_BATCHED_TOKENS="${TMX_VLLM_RELOAD_MAX_BATCHED_TOKENS:-65536}" \
      PARALLELISM="${TMX_VLLM_RELOAD_PARALLELISM:-4}" \
      RELOAD_TIMEOUT_SECONDS="${TMX_VLLM_RELOAD_TIMEOUT_SECONDS:-1800}" \
      bash tmx_jax/scripts/reload_vllm_direct_slice.sh; then
      echo "[actual-resume] vLLM SFT reload succeeded $(date -Is)"
      break
    fi
    echo "[actual-resume] WARN vLLM SFT reload failed; retrying in ${TMX_EXTERNAL_VLLM_RELOAD_RETRY_SECONDS:-60}s"
    sleep "${TMX_EXTERNAL_VLLM_RELOAD_RETRY_SECONDS:-60}"
  done
fi
...
from tmx_jax.weights import export_flax_params_to_hf_dir, load_hf_config
...
repo_id = os.environ.get("TMX_HF_CHECKPOINT_REPO", "SaifPunjwani/two-model-exploration-checkpoints")
