...
LAUNCH_RETRIES="${TMX_VLLM_BOOTSTRAP_RETRIES:-4}"
LAUNCH_RETRY_SLEEP="${TMX_VLLM_BOOTSTRAP_RETRY_SLEEP:-8}"
SKIP_FAILED_WORKERS="${TMX_VLLM_SKIP_FAILED_WORKERS:-0}"
SSH_CONNECT_TIMEOUT_SECONDS="${TMX_VLLM_SSH_CONNECT_TIMEOUT_SECONDS:-30}"
SSH_SERVER_ALIVE_INTERVAL="${TMX_VLLM_SSH_SERVER_ALIVE_INTERVAL:-10}"
SSH_SERVER_ALIVE_COUNT_MAX="${TMX_VLLM_SSH_SERVER_ALIVE_COUNT_MAX:-3}"
...
if [ "${TMX_VLLM_BATCHED_SLICE_BOOTSTRAP:-0}" != "0" ]; then
  worker_spec="$(gcloud_worker_spec "$WORKERS")"
  echo "[vllm-slice] batched bootstrap workers=[$WORKERS] worker_spec=$worker_spec batch_size=${TMX_VLLM_BATCH_SIZE:-4}"
  "${gcloud_cmd[@]}" alpha compute tpus tpu-vm scp \
    "$BOOTSTRAP_LOCAL" \
    "$TPU_NAME:~/bootstrap_vllm_worker.sh" \
    --worker="$worker_spec" \
    --batch-size="${TMX_VLLM_BATCH_SIZE:-4}" \
    --zone="$ZONE" \
    --tunnel-through-iap \
    "${scp_flags[@]}"
  "${gcloud_cmd[@]}" alpha compute tpus tpu-vm ssh \
    ...
    --command="
      ...
      TMX_VLLM_SUPERVISE='$TMX_VLLM_SUPERVISE' \
      TMX_VLLM_RESTART_SLEEP_SECONDS='$TMX_VLLM_RESTART_SLEEP_SECONDS' \
      VLLM_ENV_CACHE_GCS_URI='$VLLM_ENV_CACHE_GCS_URI' \
...
launched_workers=""
if [ "${TMX_VLLM_PARALLEL_LAUNCH:-1}" != "0" ]; then
  tmp_dir="$(mktemp -d /tmp/tmx_vllm_launch.XXXXXX)"
  pids=""
  for w in $WORKERS; do
    (
