...
while true; do
  state="$(
    ssh -o StrictHostKeyChecking=no -o ConnectTimeout=10 "$REMOTE_HOST" "bash -lc '
      ...
      echo root=\$ROOT
      if pgrep -f \"python -m tmx_jax.pipeline\" >/dev/null; then echo pipeline=alive; else echo pipeline=dead; fi
      echo latest_train:
      grep -E \"^\\[train\\] step|^\\[ckpt\\]|external reload|accepted .*correct trajectories|actual_hf_final|TMX_SKIP_FINAL_EVAL|Traceback|ERROR\" \"\$ROOT/launcher.log\" 2>/dev/null | tail -12 || true
      echo latest_ckpts:
