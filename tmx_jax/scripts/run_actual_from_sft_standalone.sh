...
RUN_NAME="${RUN_NAME:-jax_contract_lambda0_clean_20260420_080603}"
WANDB_RUN_ID="${WANDB_RUN_ID:-jaxe03b9c45e8ad3f2f18af074a}"
...
mkdir -p "$ACTUAL_DIR"
cd "$HOME/tmx-jax"
source "$HOME/miniforge3/bin/activate" tmx_jax
...
export TMX_WANDB_SINGLE_RUN=1 TMX_WANDB_COMPACT=1
export TMX_WANDB_RUN_ID="$WANDB_RUN_ID"
export TMX_WANDB_ACTUAL_GRPO_RUN_ID="$WANDB_RUN_ID"
export TMX_RESTORE_OPT_STATE=0 TMX_SAVE_OPT_STATE=0
export TMX_VLLM_RETURN_LOGPROBS=0 TMX_TRACK_EXTERNAL_OLD_GAP=0

if [[ "${TMX_ALLOW_STANDALONE_ACTUAL_WITH_PRELOADED_VLLM:-0}" != "1" ]]; then
  echo "[actual] refusing standalone Actual GRPO unless vLLM has already been reloaded to the SFT policy." >&2
  echo "[actual] Use tmx_jax.pipeline for the clean path, or set TMX_ALLOW_STANDALONE_ACTUAL_WITH_PRELOADED_VLLM=1 after verifying vLLM." >&2
  exit 2
fi
