...
LOCAL_MODEL_DIR_BASENAME="${LOCAL_MODEL_DIR_BASENAME:-tmx_vllm_model}"
PORT="${PORT:-8000}"
TP_SIZE="${TP_SIZE:-4}"
...
LAUNCH_RETRIES="${TMX_VLLM_RELOAD_LAUNCH_RETRIES:-4}"
LAUNCH_RETRY_SLEEP="${TMX_VLLM_RELOAD_LAUNCH_RETRY_SLEEP:-8}"
LAUNCH_SSH_TIMEOUT_SECONDS="${TMX_VLLM_RELOAD_LAUNCH_SSH_TIMEOUT_SECONDS:-180}"
POLL_SSH_TIMEOUT_SECONDS="${TMX_VLLM_RELOAD_POLL_SSH_TIMEOUT_SECONDS:-45}"
...
MANUAL_RELOAD_MARKER="/tmp/tmx_manual_reload_${LOCAL_MODEL_DIR_BASENAME}.done"
SKIP_FAILED_WORKERS="${TMX_VLLM_SKIP_FAILED_WORKERS:-0}"
MIN_HEALTHY_WORKERS="${TMX_VLLM_RELOAD_MIN_HEALTHY_WORKERS:-0}"
...
if [ "${TMX_VLLM_RELOAD_ALLOW_PARALLEL:-0}" != "1" ] && [ "${PARALLELISM}" -gt 1 ]; then
...
if [ "${TMX_VLLM_MULTI_SERVER:-0}" = "1" ]; then
  multi_worker_script_b64="$(base64 < "$SCRIPT_DIR/vllm_multiserver_worker.sh" | tr -d '\n')"
  multi_chips="${CHIPS:-${TMX_VLLM_MULTI_SERVER_CHIPS:-0 1 2 3}}"
...
rm -rf /tmp/tmx_external_*_w${worker} /tmp/tmx_vllm_reload_* 2>/dev/null || true
